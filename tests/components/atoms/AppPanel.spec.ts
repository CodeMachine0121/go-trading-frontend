...
describe('AppPanel', () => {
  ...
  it.each([
    ...
    const wrapper = mount(AppPanel, {
      attachTo: document.body, props, slots })
  ...
  it('三樣都沒有就不畫標題列——一條空的窄帶只是在浪費高度', () => {
    const wrapper = mount(AppPanel, {
      attachTo: document.body, slots: { default: '<p>只有內容</p>' } })
  ...
  it('收得起來的面板按一下收掉內容，再按一下拿回來', async () => {
    ...
    // 收起來時內容看不見，也就接不到鍵盤焦點——不會按著 Tab 掉進一塊已經收起來的東西裡。
    expect(wrapper.get('[data-testid="content"]').isVisible()).toBe(false)
  ...
  it('沒有給收尾那一條就不畫它', () => {
    const withoutFooter = mount(AppPanel, {
      attachTo: document.body, props: { title: '圖' } })
  ...
  describe('一打開就已經收著', () => {
    it.each([
      ...
      expect(wrapper.get('[data-testid="content"]').isVisible()).toBe(contentIsRendered)
    })
    ...
    it('收不起來的面板不會因為環境的一句話就消失', () => {
      ...
      expect(wrapper.get('[data-testid="content"]').isVisible()).toBe(true)
    })
  ...
  it('收著的時候裡面的東西仍然掛著——正在等的清單回來時，還改得了選著的那一個', async () => {
    const wrapper = mount(AppPanel, {
      attachTo: document.body,
      props: { title: '看什麼', collapsible: true, initiallyCollapsed: true },
      slots: { default: '<select data-testid="content"><option>BTCUSDT</option></select>' },
    })
