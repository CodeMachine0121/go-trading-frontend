...
describe('StrategyScriptNameDialog', () => {
  it('問的就是那兩件事：叫什麼、做什麼', () => {
    ...
    expect(wrapper.find('[data-testid="strategy-script-description-input"]').exists()).toBe(true)
  ...
  it.each([
    ...
    const wrapper = mountNameDialog()
    await wrapper.get('[data-testid="strategy-script-name-input"]').setValue(typed)
  ...
  it('重新打開時從空白開始——上一次留下的名字對這一次沒有意義', async () => {
    const wrapper = mountNameDialog()
    await wrapper.get('[data-testid="strategy-script-name-input"]').setValue('舊的名字')
  ...
  it('重新打開時回到這一次該有的起點', async () => {
    const wrapper = mountNameDialog({ title: '重新命名', initialName: '二十根均線' })
    await wrapper.get('[data-testid="strategy-script-name-input"]').setValue('打到一半的字')
    await wrapper.setProps({ open: false })
    await wrapper.setProps({ open: true })
  ...
  it('儲存中時不讓再按一次', () => {
    ...
    expect(wrapper.get('[data-testid="strategy-script-name-submit"]').attributes('disabled'))
      .toBeDefined()
  })
...
describe('StrategyScriptNameDialog：說明', () => {
  it('說明與名字一起交出去', async () => {
    // 兩者一起被想到（「這是什麼、它做什麼」），所以一起問、一起送。
    const wrapper = mountNameDialog()
    await wrapper.get('[data-testid="strategy-script-name-input"]').setValue('二十根均線')
    await wrapper.get('[data-testid="strategy-script-description-input"]').setValue('抓短線轉折')
  ...
  it('每次打開都從這一次該有的起點開始，不留上一次打的說明', async () => {
    ...
    await wrapper.get('[data-testid="strategy-script-description-input"]').setValue('改到一半的')

    await wrapper.setProps({ open: false })
    await wrapper.setProps({ open: true })

    expect((wrapper.get('[data-testid="strategy-script-description-input"]')
