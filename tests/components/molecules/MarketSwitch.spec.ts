...
describe('MarketSwitch', () => {
  it.each([
    ...
    const wrapper = mountSwitch(new MarketCounterpartDto(side, path))
  ...
  it.each([
    ...
    const wrapper = mountSwitch(new MarketCounterpartDto(side, '/somewhere'))

    expect(wrapper.get('[role="switch"]').attributes('aria-checked')).toBe(checked)
  })

  it('不分現貨合約的畫面上按不動，並說明原因', async () => {
    const wrapper = mountSwitch(new MarketCounterpartDto(null, null))
