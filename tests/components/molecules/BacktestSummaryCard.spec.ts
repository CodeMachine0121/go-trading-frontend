...
describe('BacktestSummaryCard', () => {
  ...
  it('那張卡上沒有強平出場', () => {
    // 斷言寫成缺席，因為這是它會悄悄回來的樣子：這一格上一次存在過，
    // 而它回來的時候會長得像一個合理的欄位，只是永遠寫著零。
    //
    // 驗的是那三個字而不是 testid：testid 拿掉了、標題留著，
    ...
    // 所以這不是因為整張卡沒渲染出來才過的。
    expect(wrapper.text()).toContain('止損出場')
    expect(wrapper.text()).toContain('止盈出場')
  })
