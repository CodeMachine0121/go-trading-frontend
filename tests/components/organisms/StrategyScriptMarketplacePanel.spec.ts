...
import { buildStrategyScriptMarketplaceApplication, buildStoredStrategyScript, buildAdoptedStrategyScript }
  from '../../fixtures/strategy-script-application'

/** 市集上的一支，如同後端交出來的樣子。**它沒有算式**。 */
function publishedStrategyScriptOf(
  id: number,
  name: string,
...
async function mountPanel(
  ...
  const wrapper = mount(StrategyScriptMarketplacePanel, {
    props: {
      strategyScriptMarketplaceApplication: buildStrategyScriptMarketplaceApplication(
        {
          browseMarketplace: vi.fn().mockResolvedValue([publishedStrategyScriptOf(9, '別人的')]),
          ...marketplaceProxy,
        },
        strategyScriptProxy,
      ),
...
describe('StrategyScriptMarketplacePanel', () => {
  ...
  it('沒寫說明時說一句話，而不是留一塊空白', async () => {
    ...
    expect(wrapper.get('[data-testid="marketplace-strategy-script-9"]').text())
      .toContain('分享的人沒有寫說明')
  })
  ...
  it('自己分享的那一支標明是自己的，而且一顆按鈕都不給', async () => {
    // 自己的策略腳本本來就在自己清單裡，給一顆按不出任何變化的按鈕比不給更糟。
    const wrapper = await mountPanel({}, {
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({
        mine: [buildStoredStrategyScript(9, '我分享的')],
        adopted: [],
      }),
    })
  ...
  it('那一支剛被收回時，說得出下一步是重新看一次', async () => {
    const wrapper = await mountPanel({
      adoptStrategyScript: vi.fn().mockRejectedValue(new StrategyScriptNotFoundError('找不到')),
    })
  ...
  it('空的市集不是錯誤', async () => {
    ...
    expect(wrapper.get('[data-testid="marketplace-empty"]').text()).toContain('還沒有任何策略腳本')
  ...
  it('畫不成線的那幾種先講，不等使用者套到圖上才失敗', async () => {
    const wrapper = await mountPanel({
      browseMarketplace: vi.fn().mockResolvedValue([
        publishedStrategyScriptOf(9, '訊號策略腳本', { resultType: 'signal' }),
      ]),
    })

    expect(wrapper.get('[data-testid="marketplace-strategy-script-undrawable-9"]').text())
      .toContain('畫不成線')
  })
})

describe('StrategyScriptMarketplacePanel 的搜尋', () => {
  const twoStrategyScripts = vi.fn().mockResolvedValue([
    publishedStrategyScriptOf(1, '二十根均線', { description: '抓短線轉折' }),
    publishedStrategyScriptOf(2, '布林通道', { publisherEmail: 'ming@example.com' }),
  ])

  async function search(query: string, browseMarketplace = twoStrategyScripts) {
  ...
  function visibleNames(wrapper: Awaited<ReturnType<typeof search>>): string[] {
    return wrapper.findAll('li[data-testid^="marketplace-strategy-script-"]')
  ...
  it('搜不到時說的是「沒有符合」，不是「市集上還沒有任何策略腳本」', async () => {
  ...
  it('搜出來的那一張照樣加得進來', async () => {
    const adoptStrategyScript = vi.fn().mockResolvedValue(undefined)
    const wrapper = await mountPanel({ adoptStrategyScript, browseMarketplace: twoStrategyScripts })
  ...
  it('搜尋不留存——重新打開就是全部', async () => {
    ...
    const reopened = await mountPanel({ browseMarketplace: twoStrategyScripts })
