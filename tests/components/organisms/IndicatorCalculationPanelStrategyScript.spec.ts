...
async function pickStrategyScript(wrapper: ReturnType<typeof mountPanel>, id: number) {
  await wrapper.get('[data-testid="strategy-script-picker-select"]').setValue(String(id))
  await settle()
}

/** 按下「新的空白策略腳本」——編輯器裡的「開新檔案」。 */
async function startBlankStrategyScript(wrapper: ReturnType<typeof mountPanel>) {
  await wrapper.get('[data-testid="new-strategy-script-button"]').trigger('click')
  await settle()
}
...
describe('指標計算畫面上的策略腳本：挑一支來用', () => {
  ...
  it('挑一支不會動到交易標的——策略腳本不記交易標的', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] }),
    })
    await settle()
...
describe('指標計算畫面上的策略腳本：不弄丟寫到一半的東西', () => {
  ...
  ])('只改了$changed 時不問——它不屬於任何一支策略腳本，沒有東西會被弄丟', async ({ selector, value }) => {
...
describe('指標計算畫面上的策略腳本：存回去', () => {
  ...
  it('填完名字就建立一支新的，之後使用中的就是它', async () => {
    const createStrategyScript = vi.fn().mockResolvedValue(buildStoredStrategyScript(9, '新的一支'))
    const updateStrategyScript = vi.fn().mockResolvedValue(buildStoredStrategyScript(9, '新的一支'))
    const wrapper = mountPanel({
      createStrategyScript,
      updateStrategyScript,
  ...
  it('從既有的一支另存出新的一支，原本那一支不受影響', async () => {
    const createStrategyScript = vi.fn().mockResolvedValue(buildStoredStrategyScript(8, '二十根均線 v2'))
  ...
  it('名稱被佔用時對話框不關閉、就地說明，畫面內容一字不動', async () => {
    const wrapper = mountPanel({
      createStrategyScript: vi.fn().mockRejectedValue(
  ...
  it('要存回去的那一支已經不在時說找不到，畫面內容一字不動', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] }),
      updateStrategyScript: vi.fn().mockRejectedValue(new StrategyScriptNotFoundError('找不到識別碼為 7 的策略腳本')),
    })
    await settle()
    await pickStrategyScript(wrapper, 7)
    await typeScriptBody(wrapper, '我改過的東西')
...
describe('指標計算畫面上的策略腳本：改名', () => {
  it('只有手上真的有一支時才改得了名字', async () => {
    ...
    expect(wrapper.get('[data-testid="rename-strategy-script-button"]').attributes('disabled'))
      .toBeDefined()
  })
  ...
  it('改名的框裡先放著現在的名字', async () => {
    ...
    await wrapper.get('[data-testid="rename-strategy-script-button"]').trigger('click')
    await settle()

    expect(wrapper.get<HTMLInputElement>('[data-testid="strategy-script-name-input"]').element.value)
      .toBe('二十根均線')
  })

  it('改名送出的是同一支策略腳本，只有名字換了', async () => {
    ...
    await wrapper.get('[data-testid="rename-strategy-script-button"]').trigger('click')
    await settle()
    await wrapper.get('[data-testid="strategy-script-name-input"]').setValue('均線 20')
    ...
    expect(createStrategyScript).not.toHaveBeenCalled()
    expect(updateStrategyScript).toHaveBeenCalledOnce()
    expect(updateStrategyScript.mock.calls[0]?.[0].id).toBe(7)
    expect(updateStrategyScript.mock.calls[0]?.[0].name).toBe('均線 20')
  })

  it('改名之後畫面上顯示的就是新名字', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn()
        .mockResolvedValueOnce({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] })
        .mockResolvedValue({ mine: [buildStoredStrategyScript(7, '均線 20')], adopted: [] }),
      updateStrategyScript: vi.fn().mockResolvedValue(buildStoredStrategyScript(7, '均線 20')),
    })
    await settle()
    await pickStrategyScript(wrapper, 7)
    await wrapper.get('[data-testid="rename-strategy-script-button"]').trigger('click')
    ...
    expect(wrapper.get('[data-testid="strategy-script-notice"]').text()).toContain('均線 20')
  ...
  it('改成別人用過的名字時退回改名的對話框，不是退回另存', async () => {
    ...
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] }),
      updateStrategyScript: vi.fn().mockRejectedValue(
        new StrategyScriptNameConflictError('策略腳本名稱「六十根均線」已被使用')),
    })
    await settle()
    await pickStrategyScript(wrapper, 7)
    await wrapper.get('[data-testid="rename-strategy-script-button"]').trigger('click')
    await settle()
    await wrapper.get('[data-testid="strategy-script-name-input"]').setValue('六十根均線')

    await wrapper.get('[data-testid="strategy-script-name-submit"]').trigger('click')
    await settle()

    expect(wrapper.text()).toContain('重新命名')
    expect(wrapper.text()).not.toContain('另存為新策略腳本的名稱')
    expect(wrapper.get('[data-testid="field-error"]').text()).toContain('已被使用')
    expect(wrapper.get<HTMLInputElement>('[data-testid="strategy-script-name-input"]').element.value)
      .toBe('六十根均線')
  })
  ...
  it('儲存不會順手改掉名字——那是另一個動作', async () => {
    ...
    expect(updateStrategyScript.mock.calls[0]?.[0].name).toBe('二十根均線')
  })
})
...
describe('指標計算畫面上的策略腳本：清單與刪除', () => {
  ...
  it('刪除前先問過；取消就不刪', async () => {
    const deleteStrategyScript = vi.fn()
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] }),
    ...
    await settle()
    expect(deleteStrategyScript).not.toHaveBeenCalled()
  })

  it('確認刪除之後那一支就從清單上消失', async () => {
    const listAvailableStrategyScripts = vi.fn()
      .mockResolvedValueOnce({
        mine: [buildStoredStrategyScript(7, '二十根均線'), buildStoredStrategyScript(8, '六十根均線')],
        adopted: [],
      })
      .mockResolvedValueOnce({
        mine: [buildStoredStrategyScript(7, '二十根均線'), buildStoredStrategyScript(8, '六十根均線')],
      ...
      .mockResolvedValue({ mine: [buildStoredStrategyScript(8, '六十根均線')], adopted: [] })
    const wrapper = mountPanel({
      listAvailableStrategyScripts, deleteStrategyScript: vi.fn().mockResolvedValue(undefined),
    })
    await settle()
    await wrapper.get('[data-testid="open-library-button"]').trigger('click')
    await settle()
  ...
  it('刪掉別的那一支時，正在用的那一支完全不受影響', async () => {
    ...
    const wrapper = mountPanel({ listAvailableStrategyScripts, deleteStrategyScript: vi.fn().mockResolvedValue(undefined) })
    await settle()
    await pickStrategyScript(wrapper, 7)
    await wrapper.get('[data-testid="open-library-button"]').trigger('click')
    await settle()
    await wrapper.get('[data-testid="strategy-script-library-delete-8"]').trigger('click')
    await settle()
  ...
  it('刪除時連不上後端，那一支仍在清單上並說明連不上', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] }),
      deleteStrategyScript: vi.fn().mockRejectedValue(new BackendUnreachableError('http://localhost:8080')),
    })
    await settle()
    await wrapper.get('[data-testid="open-library-button"]').trigger('click')
    await settle()
  ...
  it('打開清單時連不上後端就說連不上，不呈現空清單', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockRejectedValue(new BackendUnreachableError('http://localhost:8080')),
    })
    await settle()
    ...
    expect(wrapper.get('[data-testid="strategy-script-library-error"]').text()).toContain('連不上後端')
...
describe('指標計算畫面上的策略腳本：開一份新的空白', () => {
  ...
  it('一次後端請求都不發，清單一支不增不減', async () => {
    const listAvailableStrategyScripts = vi.fn().mockResolvedValue({
      mine: [buildStoredStrategyScript(1, '甲'), buildStoredStrategyScript(2, '乙'), buildStoredStrategyScript(3, '丙')],
      adopted: [],
    })
    const createStrategyScript = vi.fn()
    const updateStrategyScript = vi.fn()
    const deleteStrategyScript = vi.fn()
    const wrapper = mountPanel({ listAvailableStrategyScripts, createStrategyScript, updateStrategyScript, deleteStrategyScript })
    await settle()
    const listCallsBefore = listAvailableStrategyScripts.mock.calls.length

    await startBlankStrategyScript(wrapper)

    expect(listAvailableStrategyScripts.mock.calls).toHaveLength(listCallsBefore)
    expect(createStrategyScript).not.toHaveBeenCalled()
    expect(updateStrategyScript).not.toHaveBeenCalled()
    expect(deleteStrategyScript).not.toHaveBeenCalled()
    expect(wrapper.findAll('[data-testid="strategy-script-picker-select"] option')).toHaveLength(4)
  })
...
describe('指標計算畫面上的策略腳本：參數是策略腳本內容', () => {
  ...
  it('挑一支就把它記住的參數一起帶進畫面', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({ mine: [
        buildStoredStrategyScript(7, '布林通道', {
          parameters: [new StrategyScriptParameterDto('期數', 'lookbackCount', 50)],
        }),
      ], adopted: [] }),
    })
    await settle()
  ...
  it('挑一支沒有參數的策略腳本，畫面上原本那幾格跟著清掉', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] }),
    })
    await settle()
  ...
  it('另存成一支新策略腳本時，畫面上的參數跟著存進去', async () => {
    ...
    await wrapper.get('[data-testid="save-as-strategy-script-button"]').trigger('click')
    await wrapper.get('[data-testid="strategy-script-name-input"]').setValue('布林通道')
    await settle()
    await wrapper.get('[data-testid="strategy-script-name-submit"]').trigger('click')
    await settle()

    expect(createStrategyScript.mock.calls[0]![0].parameters).toEqual([
...
describe('指標計算畫面上的策略腳本：加入來的那些', () => {
  ...
  it('挑策略腳本那一排看得到兩段', async () => {
    ...
    const options = wrapper.findAll('[data-testid="strategy-script-picker-select"] option')
...
describe('指標計算畫面上的策略腳本：分享與收回', () => {
  it('分享的是眼前那一支，不必先打開清單', async () => {
    ...
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({
        mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [],
      }),
    ...
    expect(wrapper.get('[data-testid="strategy-script-notice"]').text()).toContain('分享到市集')
  })
  ...
  it('分享一支不先問——做錯了收回就好，中間沒有人失去東西', async () => {
    ...
    expect(publishStrategyScript).toHaveBeenCalledOnce()
    expect(wrapper.text()).not.toContain('都會失去它')
  })

  it('分享完不會彈出策略腳本清單——那是使用者沒有要求的東西', async () => {
    const wrapper = mountPanel({
      listAvailableStrategyScripts: vi.fn().mockResolvedValue({
        mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [],
      }),
  ...
  it('沒有使用中的那一支時按不下去——與「重新命名」同一條規則', async () => {
    ...
    expect(wrapper.get('[data-testid="share-strategy-script-button"]').attributes('disabled'))
      .toBeDefined()
  })
  ...
  it('分享過的那一支，眼前那顆變成「收回」', async () => {
    ...
    expect(wrapper.find('[data-testid="share-strategy-script-button"]').exists()).toBe(false)
  })

  it('收回要先問，而且說清楚後果', async () => {
    const withdrawStrategyScript = vi.fn().mockResolvedValue(undefined)
    const wrapper = await mountPanelWithPublished({ withdrawStrategyScript })
    await pickStrategyScript(wrapper, 7)

    await wrapper.get('[data-testid="withdraw-strategy-script-button"]').trigger('click')
    await settle()

    expect(withdrawStrategyScript).not.toHaveBeenCalled()
  ...
  it('確認之後才真的收回，而且那顆按鈕換回「分享」', async () => {
    ...
    const listAvailableStrategyScripts = vi.fn()
      .mockResolvedValueOnce({ mine: [publishedStrategyScriptRow()], adopted: [] })
      .mockResolvedValue({ mine: [buildStoredStrategyScript(7, '二十根均線')], adopted: [] })
    const wrapper = mountPanel({ withdrawStrategyScript, listAvailableStrategyScripts })
    await settle()
    await pickStrategyScript(wrapper, 7)
    ...
    expect(wrapper.find('[data-testid="share-strategy-script-button"]').exists()).toBe(true)
    expect(wrapper.find('[data-testid="withdraw-strategy-script-button"]').exists()).toBe(false)
  })

  it('分享過的策略腳本仍然改得動，而且改完還是分享狀態', async () => {
    // 另一個選擇是「改了就自動下架」。那樣的話，每次微調都要記得再按一次分享，
    // 而忘記的後果是別人手上留著一支永遠不會變好的舊版本。
    const wrapper = await mountPanelWithPublished({
      updateStrategyScript: vi.fn().mockResolvedValue(buildStoredStrategyScript(7, '二十根均線')),
    })
    await pickStrategyScript(wrapper, 7)
...
/** 一支自己的策略腳本，已經分享到市集上。 */
function publishedStrategyScriptRow(): StrategyScript {
  const stored = buildStoredStrategyScript(7, '二十根均線')
...
/** 掛起一個手上那一支已經分享出去的畫面。 */
async function mountPanelWithPublished(strategyScriptProxy: Partial<IStrategyScriptProxy> = {}) {
  const wrapper = mountPanel({
    listAvailableStrategyScripts: vi.fn().mockResolvedValue({
      mine: [publishedStrategyScriptRow()],
      adopted: [],
    }),
    ...strategyScriptProxy,
  })
