...
function numberParameter(name: string, value: number) {
  return new StrategyScriptParameterDto(name, 'number', value)
}
...
describe('新增出來的那一列', () => {
  it('名稱是空的、種類是回看根數、值是 20', () => {
    ...
    const parameters = new StrategyScriptParametersDomain([]).addingNew()
  ...
  it('加在最後面，不動既有的那幾列', () => {
    const parameters = new StrategyScriptParametersDomain([lookbackCount('期數', 50)]).addingNew()
...
describe('增刪改都回傳新的一份', () => {
  it('刪掉指定的那一列', () => {
    const parameters = new StrategyScriptParametersDomain(
  ...
  it('原本那一份一個字都沒被改到', () => {
    const original = new StrategyScriptParametersDomain([lookbackCount('期數', 20)])
...
describe('這一份哪裡不對', () => {
  it.each([
    ...
    { name: '重複的名稱', parameters: [lookbackCount('期數', 20), numberParameter('期數', 2)], expected: '參數名稱 期數 重複了，同一支策略腳本內不得重複' },
  ...
  it.each([
    ...
    expect(new StrategyScriptParametersDomain(parameters).validationMessage()).toBeNull()
  })
...
describe('每一列在畫面上該長什麼樣子', () => {
  it.each([
    ...
    const parameter = new StrategyScriptParameterDomain(new StrategyScriptParameterDto('期數', kind, 20))
  ...
  it('只有回看根數那一種會讓系統多拿 K 線', () => {
    expect(new StrategyScriptParameterDomain(lookbackCount('期數', 20)).lookbackCount()).toBe(20)
    expect(new StrategyScriptParameterDomain(numberParameter('倍數', 999)).lookbackCount()).toBe(0)
  })
})

describe('是非：仍然是一個數字，只是換一種讀法', () => {
  function booleanParameter(name: string, value: number) {
    return new StrategyScriptParameterDto(name, 'boolean', value)
  }

  it('用挑的，不是用打的', () => {
    // 一個「填 0 或 1」的數字框等於要使用者記住一個系統內部的約定。
    expect(new StrategyScriptParameterDomain(booleanParameter('只看多方', 1)).control())
      .toBe('options')
  })
  ...
  it('挑得到的是算式裡真正會寫的那兩個字', () => {
    ...
    expect(new StrategyScriptParameterDomain(booleanParameter('只看多方', 1)).valueOptions()
  ...
  it.each([
    ...
    expect(new StrategyScriptParametersDomain([booleanParameter('只看多方', value)])
      .validationMessage()).toBeNull()
  })

  it('它不是回看根數，所以不影響要拿幾根', () => {
    expect(new StrategyScriptParameterDomain(booleanParameter('只看多方', 1)).lookbackCount()).toBe(0)
  })
})
