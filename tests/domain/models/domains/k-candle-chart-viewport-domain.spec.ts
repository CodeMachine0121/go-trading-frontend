...
describe('KCandleChartViewportDomain', () => {
  describe('看多長，決定每一根涵蓋多久', () => {
    it.each([
      { name: '看三十分鐘：沒有比五分鐘更細的', visibleMinutes: 30, expectedInterval: '5m' },
      { name: '看一天：五分鐘剛好 288 根', visibleMinutes: 24 * 60, expectedInterval: '5m' },
      { name: '看兩天：五分鐘會 576 根，太擠', visibleMinutes: 2 * 24 * 60, expectedInterval: '15m' },
      { name: '看五天：十五分鐘會 480 根，仍太擠', visibleMinutes: 5 * 24 * 60, expectedInterval: '1h' },
      { name: '看一年', visibleMinutes: 365 * 24 * 60, expectedInterval: '1d' },
      { name: '看四百天：一天一根恰好 400 根', visibleMinutes: 400 * 24 * 60, expectedInterval: '1d' },
      { name: '看五百天：連一天一根都擺不下', visibleMinutes: 500 * 24 * 60, expectedInterval: '1d' },
    ])('$name', ({ visibleMinutes, expectedInterval }) => {
    ...
    it('恰好落在上限的四百天不被收回', () => {
      ...
      // 正在看 2025-07-29 到 2026-09-02（四百天），前後各多取兩百天
      expect(loadPlan.startTime.toISOString()).toBe('2025-01-10T12:00:00.000Z')
      expect(loadPlan.endTime.toISOString()).toBe('2027-03-21T12:00:00.000Z')
    })

    it('拉遠到五百天時收回四百天，結束的那一端不變', () => {
      const loadPlan = viewportSpanning(500 * 24 * 60).toLoadPlan()

      // 收回後看的與上一個案例完全相同：2025-07-29 到 2026-09-02，前後各多取兩百天
      expect(loadPlan.startTime.toISOString()).toBe('2025-01-10T12:00:00.000Z')
      expect(loadPlan.endTime.toISOString()).toBe('2027-03-21T12:00:00.000Z')
    })
  })
  ...
  describe('取資料時兩側各多取半段', () => {
    it('看兩小時就取回前後各多一小時的四小時', () => {
      ...
      expect(loadPlan.startTime.toISOString()).toBe('2026-09-02T09:00:00.000Z')
      expect(loadPlan.endTime.toISOString()).toBe('2026-09-02T13:00:00.000Z')
    })
  })
