...
function preferenceOf(remembered: Record<string, number> = {}): IStrategyScriptParameterValuePreferenceProxy {
  return {
    readValue: vi.fn((_strategyScriptId: number, name: string) => remembered[name] ?? null),
    writeValue: vi.fn(),
  }
}

function domainOf(
  declared: StrategyScriptParameterDto[], remembered: Record<string, number> = {},
...
describe('AppliedIndicatorParametersDomain：宣告是唯一的真相', () => {
  ...
  it('策略腳本不再宣告某個旋鈕時，記住的值整個消失', () => {
    // 留著它只會讓一個畫面上找不到、使用者也改不動的旋鈕繼續影響計算。
    const { domain } = domainOf([new StrategyScriptParameterDto('倍數', 'number', 2)], { 期數: 60 })
  ...
  it('讀不到記憶時全部用預設值——瀏覽器不讓存東西不是失敗', () => {
    const unreadable: IStrategyScriptParameterValuePreferenceProxy = {
...
describe('AppliedIndicatorParametersDomain：記下這一次調成什麼', () => {
  it('逐個名稱記下來', () => {
    ...
    domain.remember([lookbackCount('期數', 60), new StrategyScriptParameterDto('倍數', 'number', 2)])
