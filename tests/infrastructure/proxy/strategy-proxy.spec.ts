...
describe('StrategyProxy.createStrategy', () => {
  it('把名稱與一整段算式送到策略端點', async () => {
    ...
    await new StrategyProxy(BASE_URL, signedInSessionStorage()).createStrategy(writeDomainOf())
  ...
  it('回傳後端存下來的那一支，含它給的識別碼', async () => {
    ...
    const strategy = await new StrategyProxy(BASE_URL, signedInSessionStorage()).createStrategy(writeDomainOf())
  ...
  it('名稱已被使用時說的是名稱被佔用，不是一般的拒絕', async () => {
    ...
    const create = new StrategyProxy(BASE_URL, signedInSessionStorage()).createStrategy(writeDomainOf())
...
describe('StrategyProxy.updateStrategy', () => {
  it('打的是那一支策略自己的路徑', async () => {
    ...
    await new StrategyProxy(BASE_URL, signedInSessionStorage()).updateStrategy(writeDomainOf(7))
  ...
  it('找不到那一支時說的是找不到', async () => {
    ...
    const update = new StrategyProxy(BASE_URL, signedInSessionStorage()).updateStrategy(writeDomainOf(7))
  ...
  it('名稱撞到別的策略時說的是名稱被佔用', async () => {
    ...
    await expect(new StrategyProxy(BASE_URL, signedInSessionStorage()).updateStrategy(writeDomainOf(7)))
...
describe('StrategyProxy.deleteStrategy', () => {
  it('打的是那一支策略自己的路徑', async () => {
    ...
    await new StrategyProxy(BASE_URL, signedInSessionStorage()).deleteStrategy(7)

    expect(fetchMock).toHaveBeenCalledWith(
      'http://localhost:8080/strategies/7', { headers: SIGNED_IN_HEADERS, method: 'DELETE' })
  })

  it('找不到那一支時說的是找不到', async () => {
    vi.stubGlobal('$fetch', vi.fn().mockRejectedValue(buildFetchError({ status: 404 })))

    await expect(new StrategyProxy(BASE_URL, signedInSessionStorage()).deleteStrategy(7))
...
describe('StrategyProxy 其餘的拒絕', () => {
  it.each([
    ...
    const rejection = act(new StrategyProxy(BASE_URL, signedInSessionStorage()))
  ...
  it.each([
    ...
    await expect(act(new StrategyProxy(BASE_URL, signedInSessionStorage()))).rejects.toBeInstanceOf(BackendUnreachableError)
  })
})
...
describe('StrategyProxy：策略記著的旋鈕', () => {
  it('存下去時把旋鈕一起送出，預設值就是畫面上那個數字', async () => {
    ...
    await new StrategyProxy(BASE_URL, signedInSessionStorage()).createStrategy(new StrategyWriteDomain(new StrategyWriteDto(
      '布林通道',
