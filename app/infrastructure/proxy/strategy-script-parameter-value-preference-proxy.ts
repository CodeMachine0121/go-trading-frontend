 ...
 * 那與「還沒調過」對使用者是同一件事：用策略腳本記著的預設值。
 * 寫不進去也一樣不該讓畫面停住：這一次已經用調過的值算完了，只是下次打開會回到預設。
 *
 * 讀回來的東西不是數字時也當成沒調過：儲存裡放的是文字，
 * 而別的版本、別的分頁、甚至使用者自己都可能在那裡留下讀不成數字的東西。
 */
export class StrategyScriptParameterValuePreferenceProxy
implements IStrategyScriptParameterValuePreferenceProxy {
  readValue(strategyScriptId: number, parameterName: string): number | null {
    try {
      const stored = localStorage.getItem(this.storageKeyOf(strategyScriptId, parameterName))
      if (stored === null) {
        return null
      }
  ...
  writeValue(strategyScriptId: number, parameterName: string, value: number): void {
    try {
      localStorage.setItem(this.storageKeyOf(strategyScriptId, parameterName), String(value))
    }
    catch {
