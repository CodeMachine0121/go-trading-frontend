...
import { PublishedStrategyScriptDomain } from '~/domain/models/domains/published-strategy-script-domain'

/**
 * Entity：市集上的一支策略腳本，如同系統交出來的樣子。
 *
 * **它沒有 `script` 欄位，那正是重點。** 分享出去的是一支策略腳本的用處，不是它的作法——
 * 而「別人看不到算式」在這裡不是一條要遵守的規則，是一個沒有地方可以放算式的型別。
 * 想把它載進編輯器的那段程式碼**編不過**，不必靠審查抓。
 *
 * 它也沒有指標算式以外的取數計畫：要多粗、要幾根、在哪個市場上算，一律由執行當下指定。
 */
export class PublishedStrategyScript {
  constructor(
  ...
  toDomain(): PublishedStrategyScriptDomain {
    return new PublishedStrategyScriptDomain(this)
  }
}
