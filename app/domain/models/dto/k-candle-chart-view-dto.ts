 ...
 * - `visibleRange` 是使用者**應該**看到的那一段。它可能與問的時候不一樣——
 *   拉得比最粗的刻度所能涵蓋的還遠時會被收回上限。下一次要不要重新取、
 *   指標算哪一段，說的都是它。
 ...
 * 那一段（`visibleRange`）分兩次回答，就會有一次它們不同步——
...
export class KCandleChartViewDto {
  constructor(
    public readonly visibleRange: ChartVisibleRangeVo,
