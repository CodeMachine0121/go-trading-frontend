...
/**
 * Domain Model：這一次套用要用的那幾格——策略腳本的宣告，配上這台瀏覽器記得的值。
 *
 ...
 * 上面兩條自然涵蓋：舊名字丟掉，新名字沒有記憶所以用策略腳本的預設值。
 *
...
export class AppliedIndicatorParametersDomain {
  constructor(
    ...
    private readonly declaredParameters: readonly StrategyScriptParameterDto[],
  ...
  toDtos(): StrategyScriptParameterDto[] {
    return this.declaredParameters.map((parameter) => {
      const rememberedValue = this.strategyScriptParameterValuePreferenceProxy.readValue(
        this.strategyScriptId, parameter.name)

      return new StrategyScriptParameterDto(
        parameter.name,
        parameter.kind,
        // 策略腳本帶進來的那一份，它的值就是這支策略腳本記著的預設值。
  ...
  remember(parameters: readonly StrategyScriptParameterDto[]): void {
    for (const parameter of parameters) {
      this.strategyScriptParameterValuePreferenceProxy.writeValue(
        this.strategyScriptId, parameter.name, parameter.value)
    }
  }
}
