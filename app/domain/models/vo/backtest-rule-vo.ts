...
export const BACKTEST_RULES: BacktestRuleVo[] = [
  ...
  new BacktestRuleVo(
    ...
    + '只做空是槓桿做多的鏡像：賣出開空倉，買入就平倉把錢收回來、之後空手等下一個賣點；'
    + '空手時買入什麼都不做，永遠不開多。這段時間只想做空就選它——'
    + '不要用多空反手配一個永遠不成立的買入條件去湊，那份規則會寫著兩邊都做、'
    + '實際上只做空，過幾個月連你自己都看不出來。'),
  new BacktestRuleVo(
    '同一時間只有一個倉位',
    '空手聽到開倉的那個信號，就開這套模式做得了的那一邊；已經持有同方向又聽到同一個信號，'
    + '當作沒聽到。哪一個信號開倉、哪一個平倉，由上面那條交易模式決定——'
    + '只做空剛好相反：它的賣出開空倉，買入是平倉，空手時聽到買入什麼都不做。'),
  new BacktestRuleVo(
    '押不下去就跳過，不算失敗',
