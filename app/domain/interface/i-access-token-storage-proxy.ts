...
/**
 * 介面以「能力」命名，不以供應商命名：這個能力是「記住這台瀏覽器的登入憑證」。
 * 目前由瀏覽器儲存實作；改記在 cookie（好讓伺服器端也判斷得出來）時，
 * 介面一個字都不必改。實作在 app/infrastructure/proxy/access-token-storage-proxy.ts。
 *
 * **三個方法都不會拋。** 記不住、讀不到、清不掉，對使用者都不是需要處理的錯誤：
 * 讀不到等同還沒登入過；記不住只代表下次打開要重登，而這一次仍然操作得起來。
 */
export interface IAccessTokenStorageProxy {
  /** 讀回記住的憑證；沒有記住、或記著的東西已經壞掉時回傳 null。 */
