...
- **開一份新的空白策略腳本時**，可編輯區預先放一個空的 `Calculate` stub，回傳型別跟著目前選的指標值種類。
...
**新的空白策略腳本帶一個 Calculate stub**

- 第一次進畫面、以及按「開新的空白策略腳本」時，可編輯區是一個空的 `Calculate`：
  簽章那一行 ＋ 一行空的縮排 ＋ 收尾 `}`。
...
- 載入一支策略腳本時，唯讀區照舊；可編輯區是那支策略腳本存下來的算式**去掉最上面唯讀那一塊**之後的全部。
- 認不出最上面那一塊時，整段原樣帶入並告知（沿用既有規則）。
- 之前用舊編輯器存的策略腳本，可編輯區會顯示成「`func Calculate(...) {` ＋ 內容 ＋ `}`」——正是新模型要的樣子，不需要遷移。
...
### Rule: 開新空白策略腳本帶一個 Calculate stub

| # | Given (only relevant data) | When | Then |
|---|---|---|---|
| 1 (happy) | 指標值種類是「一個信號」 | 開一份新的空白策略腳本 | 可編輯區是 `func Calculate(data []indicator.KCandle) indicator.Signal {`、一行空縮排、`}` |
...
| # | Given (only relevant data) | When | Then |
|---|---|---|---|
| 1 (happy) | 一支策略腳本存的算式是「唯讀區 ＋ 一整個 Calculate 函式」 | 載入它 | 可編輯區是那一整個 Calculate 函式；唯讀區照舊 |
| 2 (happy) | 一支用舊編輯器存的策略腳本（外框含 `func Calculate` 簽章那一行） | 載入它 | 可編輯區顯示成 `func Calculate(...) 形狀 { 內容 }`，不需遷移 |
| 3 (exception) | 一支策略腳本的算式最上面那一塊認不出來 | 載入它 | 整段原樣帶入，並告知認不出外框 |
