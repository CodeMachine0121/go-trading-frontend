...
Oracle: Acceptance Criteria（20 個情境）+ Core Business Rules（8 條）+ Non-Functional（4 條）
...
| AC-19 | 開始時間被清空 | 不進行查詢，提示「請填寫開始時間」 | `utc-time-format.ts:8-20`（格式不合回無效時間）＋`k-candle-query-domain.ts:23-25` | `k-candle-query-domain.spec.ts:41`、`KCandleSearchPanel.spec.ts:91`（第四列） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-20 | 結束時間被清空 | 不進行查詢，提示「請填寫結束時間」 | 同上（`:27-29`） | `k-candle-query-domain.spec.ts:41`、`KCandleSearchPanel.spec.ts:91`（第五列） | asserts-oracle | produces-oracle | ✅ conforms |
...
| BR-3 | 條件合法性：標的去空白後不得為空；起訖都必須完整填寫；結束不得早於開始；相等為合法 | 四條規則同時成立 | `k-candle-query-domain.ts:15-40` | `k-candle-query-domain.spec.ts:10/27/41/56/65` | asserts-oracle | produces-oracle | ✅ conforms |
...
靜態稽核是拿「規格 vs 測試 vs 程式碼」三方比對，它抓不到「**測試替身與真實世界形狀不同**」
這類問題——這兩個都是 PR review 階段才發現的：

1. **後端沒啟動會被誤判成後端拒絕**（HIGH）。底層的 `FetchError` **一律**帶著 `response` 屬性
   （連不上時值為 `undefined`），原本用 `'response' in error` 判斷因此恆為真。
   當時的測試之所以綠，是因為替身是自己 `new` 出來的 Error，沒有那個永遠存在的屬性。
   修正：改判斷「有沒有回應物件」，並把測試替身換成真正的 `FetchError`。
2. **時間欄位被清空會靜默查到西元 2000 年**（HIGH，比 review 判斷的更嚴重）。
   `new Date(':00Z')` 不是無效日期，而是 **2000-01-01**；清空開始時間會查一個完全不對的區間，
   清空結束時間則得到誤導性的「結束時間不得早於開始時間」。
   修正：時間輸入的格式在轉換當下就檢查，不合格式回傳無效時間值，由 domain 明確拒絕並指出欄位。
