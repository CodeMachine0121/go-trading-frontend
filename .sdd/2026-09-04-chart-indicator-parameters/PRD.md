...
策略腳本已經記得住自己的旋鈕，但那件事只在**寫算法的地方**做完了。
真正會反覆調那幾個數字的地方是**圖表**——看行情的人想把同一支均線同時擺上二十期與
六十期看它們什麼時候交叉，想把布林的倍數從 2 調到 2.5 看價格有沒有跑出去。

今天他做不到兩件事：**調不了**（圖上一律用策略腳本記著的預設值），
以及**擺不了兩次**（同一支策略腳本只能套用一次）。於是他只剩一條路：
回到寫算法的地方把預設值改掉、存檔、回到圖表、再看一次，然後為了看另一個值再來一輪——
而且他的策略腳本被自己改過了，明天打開別的圖會發現它不是原本那支。

### 目標

在圖表上套用一支策略腳本時，**先調它的旋鈕再讓它上圖**；**同一支可以擺好幾次**，
每一次配自己的值、各自一條線；調過的值**這台瀏覽器記得住**。
調的是「這一次」，**策略腳本記著的預設值一個字都不會變**。

### 成功的樣子

- 使用者要同時看二十期與六十期兩條均線，**不必回到寫算法的地方，也不必改動那支策略腳本**。
- 他昨天調過的值，今天挑同一支策略腳本時**已經在那幾格裡**。

### 明確不做

- 在圖表上編輯策略腳本的**旋鈕宣告**（新增／刪除／改名／改預設值）——那是寫算法的地方的事。
- 把**已套用的清單**存起來：重新打開畫面後清單仍然是空的。
- 旋鈕的**型別系統**（字串、是非、選單）：仍然只有回看根數與數值兩種。
- 把同一支策略腳本的多次套用**群組起來呈現**（摺疊、共用一組顏色）。
...
| Persona | 是誰 | 在什麼情境下 |
| :--- | :--- | :--- |
| **看行情的人** | 這個系統唯一的使用者，本人 | 盯著 K 線圖，想在同一張圖上比較同一套算法在不同參數下的樣子。他要的是**這一次想這樣看**，不是改那支策略腳本 |
...
**As a** 看行情的人，**I want** 在策略腳本上圖之前先把它的旋鈕調成這一次要的值，
**so that** 圖上出現的第一條線就是我要看的那一條。

```gherkin
Scenario: 挑一支有旋鈕的策略腳本，先看到那幾格
  Given 一支策略腳本宣告了「期數」，預設值是 20
  When 挑它
  Then 出現一格「期數」，裡面是 20
  And 它還沒上圖
...
Scenario: 一個旋鈕都沒有的策略腳本挑了就直接上圖
  Given 一支一個旋鈕都沒有宣告的策略腳本
  When 挑它
  Then 它直接上圖並立刻算了一次
  And 中間沒有多出任何一步

Scenario: 上次調過的值就是這次那一格的起點
  Given 這台瀏覽器上次把某支策略腳本的「期數」調成 60
  When 再挑那一支
  Then 那一格是 60

Scenario: 這台瀏覽器不讓網站存東西時照樣調得動
  Given 這台瀏覽器不讓網站存東西
  And 某支策略腳本的「期數」預設值是 20
  When 挑它
  Then 那一格是 20
  And 調得動、也算得出來，畫面沒有出現任何錯誤

Scenario: 在圖上調過的值不會改動策略腳本記著的預設值
  Given 某支策略腳本的「期數」預設值是 20
  And 在圖上把它調成 60 並加入
  When 回到寫算法的地方看那支策略腳本
  Then 它記著的預設值仍然是 20
```

### US-02 — 同一支可以擺好幾次 [priority: P0]

**As a** 看行情的人，**I want** 把同一支策略腳本配不同的值擺上好幾次，
**so that** 我能在同一張圖上比較它們。

```gherkin
Scenario: 已套用的策略腳本仍然挑得到
  Given 已套用「均線」，期數 20
  When 打開可挑清單
  Then 「均線」仍然在可挑的選項裡
...
Scenario: 沒有旋鈕的策略腳本也可以擺兩次
  Given 一支一個旋鈕都沒有的策略腳本
  When 套用它兩次
...
Scenario: 同一支被調過兩個值時，記住的是最後設定的那一個
  Given 同一支策略腳本被套用兩次，期數分別設成 20 與 60，60 是後設的
  When 重新打開畫面並挑那一支
  Then 那一格是 60
```

### US-04 — 策略腳本的旋鈕被改過之後 [priority: P0]

**As a** 看行情的人，**I want** 策略腳本的宣告改過之後圖上以宣告為準，
**so that** 一個已經不存在的旋鈕不會繼續影響計算。

```gherkin
Scenario: 策略腳本多宣告了一個旋鈕
  Given 這台瀏覽器記住某支策略腳本的「期數」是 60
  And 那支策略腳本現在多宣告了「倍數」，預設值 2
  When 挑它
  Then 「期數」那一格是 60
  And 「倍數」那一格是 2

Scenario: 策略腳本不再宣告某個旋鈕
  Given 這台瀏覽器記住某支策略腳本的「期數」是 60
  And 那支策略腳本現在不再宣告「期數」
  When 挑它
  Then 沒有「期數」那一格
...
Scenario: 同一支的第二次套用不沿用記住的顏色
  Given 替某支策略腳本的線挑過藍色，而那條線還在圖上
  When 再套用同一支一次
  Then 第二條線不是藍色
  And 它是一個目前沒有被用掉的顏色

Scenario: 沒有別條線在用它時，記住的顏色回來
  Given 同一支策略腳本被套用兩次，第一次是藍色
  When 移除第一筆之後再套用一次
  Then 那條線是藍色
...
```mermaid
flowchart TD
    A[挑一支已存策略腳本] --> B{它有宣告旋鈕嗎}
    B -- 沒有 --> E[直接上圖並算一次]
    B -- 有 --> C[出現那幾格，帶著起始值]
    C --> C1[起始值：這台瀏覽器上次留下的；沒有就用策略腳本的預設值]
    C1 --> D[使用者調成這一次要的值]
    D --> E
    E --> F{算得出來嗎}
...
- **調的是「這一次」。** 圖上填的值只屬於這一次套用，不改動策略腳本記著的預設值。
- **每一次套用是獨立的一筆。** 參數值、線色、失敗說明、移除、重算，全都只影響那一筆。
- **同一支策略腳本可以套用任意多次**，靠**參數值**分辨——那是它們唯一的差別。
- **記住的是「這支策略腳本的這個旋鈕上次被調成什麼」**，不是「上次擺了哪幾條線」。
- **宣告是唯一的真相**：策略腳本現在宣告什麼就有哪幾格；記住的值裡名字不在的丟掉，
  名字還在的沿用，新多出來的用策略腳本的預設值。
...
- **挑策略腳本之後、加入之前**，那幾格與加入的動作出現在可挑清單附近——
  使用者的視線不必離開他剛剛挑的那一支。
- **已套用清單的每一筆**帶著：策略腳本名稱、這一次的參數值、它畫出的那幾條線與各自的顏色、
  移除的動作。有旋鈕的那幾筆可以就地改值。
- **狀態**：
  - **計算中** — 那一筆標示計算中，不擋住其他筆。
...
- **記不住不是失敗**：這台瀏覽器不讓網站存東西時，一切照常運作，
  只是下次回到策略腳本記著的預設值；不得出現任何錯誤畫面。
- **不新增對外往返**：策略腳本的旋鈕宣告隨策略腳本清單一起來，挑一支不必再問一次。
...
- **上一個切片**（`.sdd/2026-09-04-indicator-calculation-parameters/`，已完成）：
  策略腳本帶著自己的旋鈕、計算時可以帶參數值、名字對不上是一種可以被指名的失敗。
- **系統那一側**（`go-trading` 的 `feat/strategy-script-parameters`，**尚未併入主線**）：
  策略腳本的旋鈕與帶參數值的計算都在那裡。**本地測試時系統那一側必須切到那個分支執行**，
...
- **「挑過的顏色即使被用掉也照樣採用」在同一支套用多次時破例。**
  ...
  代價：使用者可能預期「這支策略腳本永遠是藍色」而看到第二條不是——
...
「策略腳本記著的預設值不變」與「沒有旋鈕的策略腳本也可以擺兩次」的邊界）。
