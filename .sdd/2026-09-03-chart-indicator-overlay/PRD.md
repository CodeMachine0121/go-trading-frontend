...
- **Problem Statement:**
  策略腳本算得出數字，但那些數字只出現在一張表上。看行情的人要知道的不是數字是多少，
  而是**它落在圖上的哪裡**——均線壓不壓著價格、支撐位離現在多遠。表格答不出這件事。

- **Expected Outcome:**
  在 K 線圖表上挑一支已存策略腳本，系統立刻對眼前這段行情算一次並畫在同一張圖上；
...
- **Primary Role(s):** 看行情、並且自己寫策略腳本的人（本專案作者本人）。
...
### US-01 — 挑一支策略腳本套上去 [priority: P0]

**As a** 看行情的人，**I want** 從已存策略腳本中挑一支加到圖上，
**so that** 我不必再對著一張表自己想像那個數字在圖的哪裡。

```gherkin
Scenario: 挑一支就立刻算並畫出來
  Given 已套用的指標清單是空的
  And 有一支名為「二十根均線」的策略腳本
  When 把「二十根均線」加進來
  Then 它出現在已套用的指標清單上
...
Scenario: 一支策略腳本都還沒存過
  Given 系統裡一支策略腳本都沒有
  When 打開可挑的策略腳本清單
  Then 明說還沒有任何策略腳本，而不是呈現一個空選單
```
...
# 以下兩個情境已被 .sdd/2026-09-03-live-chart-updates/ 取代：
# 重算的觸發已改為「顯示區間變了」，而不是「要不要重新取一批 K 線」。
# 理由見該切片 PRD 的 Dependencies & Risks。
...
```gherkin
Scenario: 一個數字畫成水平線
  Given 一支產出一個數字 115、指標名稱為「均價」的策略腳本
  When 套用它
  Then 圖上出現一條位置在 115 的水平線
  And 那條線標著「均價」

Scenario: 一串數字畫成跟著 K 線走的曲線
  Given 一支產出一串數字的策略腳本，值的個數與圖上的 K 線根數相同
  When 套用它
  Then 圖上出現一條曲線
  And 第 n 個值畫在第 n 根 K 線的位置上

Scenario: 值比圖上的 K 線少時靠右對齊
  Given 一支產出一串數字的策略腳本，值的個數比圖上的 K 線少
  When 套用它
  Then 最後一個值畫在最後一根 K 線上，往前逐一對回去
  And 最前面那幾根沒有點，不補值

Scenario: 一支算式產出好幾個指標名稱就畫好幾條線
  Given 一支一次產出「最高」與「最低」兩個名稱的策略腳本
  When 套用它
  Then 圖上出現兩條各自獨立的線
  And 兩條線分別標著「最高」與「最低」

Scenario: 是非類型的策略腳本挑不到
  Given 一支指標值種類為一個是非的策略腳本
  When 打開可挑的策略腳本清單
  Then 那一支被標明畫不出來
  And 它不能被挑選

Scenario: 一個指標名稱都沒產出不是失敗
  Given 一支一個指標名稱都沒放進結果的策略腳本
  When 套用它
...
Scenario: 同一支策略腳本畫出的兩條線是不同顏色
  Given 一支一次產出兩個指標名稱的策略腳本
  When 套用它
  Then 那兩條線是不同的顏色
...
```mermaid
flowchart TD
    A[圖上那批 K 線換了<br/>或使用者加了一支策略腳本] --> B{已套用的指標清單是空的?}
    B -- 是 --> Z[什麼都不做]
...
- **套用的單位是策略腳本**，畫線的單位是**指標名稱**——一支策略腳本可畫出好幾條線。
- **同一支策略腳本不重複套用**；可挑清單排除已套用的那些。
- **是非類型不可套用**：可挑清單上標明並停用。
- ~~**重算的觸發是「圖上那批 K 線換了」**，沿用系統既有的「要不要重新取」那個答案。~~
  **已被 `.sdd/2026-09-03-live-chart-updates/` 取代**：觸發改為「顯示區間變了」。
- **每一支各自成敗**：一支失敗只標在它自己旁邊，並收掉它上一輪的線。
- **顏色的身分是「策略腳本＋指標名稱」**，記在這台瀏覽器上；沒挑過時依序取一個目前沒被用掉的；
  挑過的優先於避開重複。
...
- 已套用清單以策略腳本為一列；該策略腳本畫出的每一條線是其下的一行：
  顏色色票、指標名稱、換色的選單。策略腳本層級有一顆移除鈕。
- 正在算的那一支先以「計算中」呈現；失敗說明就地標在該策略腳本那一列。
...
- **External Dependencies:** 前一個切片（策略腳本與執行參數分離）已完成，
...
- 需求共識：`BRIEF.md`（同一資料夾）
- 前置切片：`.sdd/2026-09-03-strategy-script-execution-parameters/`
