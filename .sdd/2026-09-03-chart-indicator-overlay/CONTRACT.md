...
## 5. 已被後續切片取代的條款（2026-09-04）

`.sdd/2026-09-03-live-chart-updates/` 取代了本切片的兩條驗收條件。
**這不是回歸，是刻意的行為變更**，理由記在該切片的 PRD。

| 本切片的條款 | 現況 |
| :--- | :--- |
| US-02「換到需要重新取 K 線的區間時重算」 | 觸發改為**顯示區間變了**，不再看有沒有重新取資料 |
| US-02「圖上那批 K 線沒換就不重算」 | 條件收窄為**顯示區間真的沒變** |

原本的理由是「同一批資料算出來必然相同」。那句話只對**吃固定根數的滾動指標**成立——
二十期均線看的是最後二十根，往左看不改變那二十根是哪二十根；
但**一段區間內的極值**換一段就該變。把一條只對某類指標成立的性質當成通則，
另一類指標就會看起來壞掉：數字停著，而且不會有任何地方報錯。
