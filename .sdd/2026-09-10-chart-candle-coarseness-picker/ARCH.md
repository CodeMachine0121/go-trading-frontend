...
- **Code review 之後改掉的三處：**
  - **`KCandleChartPanel` 記下「使用者最後要求的那一段」**（`requestedStartTime/EndTime`），
    與「畫出來的那一段」分開。原本重試一律沿用後者，而後者只在成功時才寫，
    於是「被拒絕 → 改用更粗的一種救回來」（PRD US-05 明訂的出路）會拿回
    上一次成功的範圍，畫面上卻還亮著他按的那一個快捷區間。
  - **指標要不要重算改由呼叫端明說「有沒有換一批」**，不再從欄位推。
    推的那份清單永遠列不完：補齊填的是涵蓋範圍**之內**的洞，換回來一批多了幾根、
    交易標的／粗細／那一段卻一模一樣。順帶讓那個判斷從三個比對縮成一個事實。
  - **選單包進 `<label>`**：另外兩組裝的是按鈕（自己說得出名字），下拉說不出來。
- **Known debt / deferred:**
  - 「長度變化兩成半就重取」在挑了固定粗細時是多餘的。
    **該回頭處理的訊號：** 有人抱怨「我只是微調一下，圖就閃一次」。
  - 指標現在**每換一批就重算一次**（含補齊、換標的、換粗細、換到要重取的區間）。
    那是正確的，但沒有「這一批其實跟上一批一樣」這個近路。
    **該回頭處理的訊號：** 有人抱怨換標的又換回來時線閃兩次。
