...
實作完成後對本切片的檔案做了一次深模組檢視。**最有價值的產出不是重構，是一個真的 bug。**

**採納並做掉的：取行情失敗時要停止跟盤。**
失敗那條路徑清掉了圖與線，卻留著上一檔的跟盤。下一則即時更新會把圖「復活」——
而畫面上同時顯示著取行情失敗，看到的人會以為那張圖是這一檔的。
順帶補上「這是第幾次跟盤」的號碼：回呼是個閉包，可能比自己的訂閱活得更久，
只認自己那一次的號碼就不必假設「停止」在每一種情況下都立刻生效。
圖表與指標各自早就有同一套機制，這是第三次用同一個想法。
