...
Oracle: Acceptance Criteria（21 個情境）+ Core Business Rules（7 條）+ Non-Functional（4 條）
...
| BR-6 | 失敗分兩類 | 算式的問題與請求的問題呈現方式不同 | `indicator-calculation-proxy.ts:52-54`、`IndicatorCalculationPanel.vue:154/162` | `IndicatorCalculationPanel.spec.ts:113/126`（互斥斷言） | asserts-oracle | produces-oracle | ✅ conforms |
