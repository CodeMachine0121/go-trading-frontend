...
| AC-04 | K 線清單的起始時間照選定的時區 | UTC 04:00 的那一根在台北顯示 12:00，欄位標題標出位移 | `KCandleTable.vue:42`、`:85` | `KCandleTable.spec.ts`（UTC 與台北兩列） | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-06 | 圖表的時間也照選定的時區 | 時間軸與已取回區間都以選定時區呈現 | `KCandleChart.vue:192-196`（`localization` 與 `tickMarkFormatter`）、`KCandleChartPanel.vue:232` | `KCandleChart.spec.ts`（十字準星與刻度的兩個時區、四種刻度粗細、換時區只重講不重畫）、`KCandleChartPanel.spec.ts`（涵蓋區間換時區後改寫且不重新取） | asserts-oracle | produces-oracle | ✅ conforms |
