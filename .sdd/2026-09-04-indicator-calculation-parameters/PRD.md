...
- **Expected Outcome:**
  使用者在這個畫面上宣告策略腳本的參數、就地調整它們，並且**再也不必回答「要幾根」**——
  他說的是「我要看最近一小時」，剩下的系統自己算。

- **Out of Scope:**
  - K 線圖表上的參數（每一份各自的值、同一支策略腳本套用多次、顏色怎麼分）。
  - 是非、下拉選單之類的參數種類。
  - 由系統去讀算式、猜出它需要哪些參數。
  - 參數之間互相依賴的規則。
  - 在畫面上把「這一次的值」與「存起來的預設值」分成兩格。
  - 彙總刻度的既有規則（它仍屬於這一次計算，不跟著策略腳本存）。
...
### US-01 — 在這裡宣告策略腳本的參數 [priority: P0]

**As a** 寫策略腳本的人，**I want** 在寫算式的地方宣告它有哪些旋鈕，
...
Scenario: 參數跟著策略腳本一起存
  Given 已宣告「期數」是回看根數、預設 20
  When 把它存成一支策略腳本
  Then 那個參數連同種類與預設值都被存起來

Scenario: 載入策略腳本時參數跟著換
  Given 一支已存的策略腳本帶著「期數」與「倍數」
  When 載入它
...
- **參數是策略腳本內容**，與算式內容、指標值種類同一個層級：載入時跟著換，
  改動算是還沒存的東西。
- **參數種類只有兩種**：回看根數與數值。
...
- **Security:** 無新增。參數只影響這一次計算與這支策略腳本記著的內容。
- **Compatibility:** 無新增依賴。
...
- **系統那一側的策略腳本參數能力**（go-trading 的 `.sdd/2026-09-04-strategy-script-parameters/`）。
...
- **「畫面上只有一組數字」在這個畫面成立，在圖表上不成立。**
  ...
  但在圖表上套用的是一支**已經存好**的策略腳本，那時「這一次調成 50」不該改動它的預設值。
...
- 需求共識：`BRIEF.md`（同一資料夾）
- 系統那一側：go-trading 的 `.sdd/2026-09-04-strategy-script-parameters/`
- 下一個切片（圖表那一側）：尚未建立
- 通用語言：`.sdd/UL-MAP.md`
