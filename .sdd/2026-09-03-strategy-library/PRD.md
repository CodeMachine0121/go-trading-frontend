...
### US-10 — 替一支策略改名 [priority: P0]

**As a** 策略研究者, **I want** 只把目前這一支的名字換掉,
...
Scenario: 改成別支用過的名字
  Given 已有一支名為「六十根均線」的策略
  And 目前使用中的策略是「二十根均線」
  When 使用者把它改名為「六十根均線」
  Then 改名被拒絕，並就地說明該名稱已被使用
  And 畫面仍停在改名的地方，不是另存為新策略的地方
  And 剛打的名字還在

Scenario: 沒有使用中的策略時改不了名
  Given 沒有使用中的策略
  When 使用者查看可以做的事
  Then 改名這件事不可用
...
**As a** 策略研究者, **I want** 這些動作以圖示呈現且說得出自己叫什麼,
...
Scenario: 動作以圖示呈現
  Given 使用者在指標計算畫面上
  When 使用者看策略那一列
  Then 儲存、另存為新策略、重新命名與策略清單都以圖示呈現
...
Scenario: 動作與策略選單對齊
  Given 使用者在指標計算畫面上
  When 使用者看策略那一列
  Then 動作與策略選單在同一列並且對齊，不會錯開一截

Scenario: 一支策略都沒有時動作仍然在
  Given 系統一支策略都沒有
  When 使用者看策略那一列
...
- **動作**：儲存、另存為新策略、重新命名、開啟清單，**以圖示呈現並與策略選單同一列對齊**。
