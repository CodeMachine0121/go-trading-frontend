...
- **Problem Statement:**
  這個操作台對任何打開它的人一視同仁。網址一貼，K 線、策略腳本、要花錢的助手全部攤開，
...
- **Out of Scope:**
  ...
  - 替 K 線／指標／策略腳本／助手的呼叫附上憑證。後端目前不問那些請求來者是誰；
...
| 正在確認既有憑證 | **沒有為它做任何處理，而且實際上會閃一下操作台。** 伺服器算頁面時碰不到瀏覽器儲存（把關只在瀏覽器端跑），所以沒登入的人走到某一頁時，會先看到伺服器算好的那一頁，等到瀏覽器接手、問完後端才被帶到登入畫面。這是「把關只在瀏覽器這一側」的代價，寫出來而不是假裝它不存在 |
...
- **Known Risks:**
  ...
  - **這道門只在瀏覽器這一側。** 後端的 K 線／策略腳本／助手端點目前仍然不問來者是誰，
