...
| BR-10 | 可用策略是空的時候說得出要先有一支 | 不是給一個空的下拉選單 | `StrategyBotSignalSourceFields` 的 `signal-sources-empty` 說了來源為空，**但沒有說「先去寫一支會吐信號的策略」** | — | 🟠 **見下** |
...
✅ 46 conforms · 🔴 0 violations · 🟠 1 mis-asserted · 🟡 3 partial · ❌ 0 gaps · ⚠️ 0 orphans
Conformance: 92%
```

### 🟠 BR-10 — 可用策略是空的時候，畫面沒說出真正的下一步

PRD 第 4 節寫著：「**可用策略是空的**：說得出要先有一支會吐信號的策略，
而不是給一個空的下拉選單。」

實作只做了一半：沒有信號來源時說「這台機器人還沒有任何信號來源」，
但**一支可用策略都沒有時，新增來源的按鈕仍然在，按下去會得到一個空的策略選單**。
使用者在這裡卡住時，需要知道的是「先去寫一支信號種類的策略」——
而這個畫面一個字都沒提。

> 這是這一輪稽核找到的唯一實質落差。它不是程式壞了（送出去仍然會被擋），
> 而是**畫面在一個它明明知道原因的地方保持沉默**。修法很小：
> `strategyOptions` 為空時，把新增鍵換成那一句話與一條到策略庫的路。
> 依既定作法 `/contract` 只稽核不改碼，留給你拍板。
