...
- **Primary Role(s):** **使用者**——這台操作台的主人，同時是寫那幾支策略腳本的人。
  單人使用，本機或內網，已登入。
- **Usage Context:**
...
Scenario: 停擺的那一台特別顯眼,並說出原因
  Given 一台因為「某一支策略腳本找不到了」被系統停下來
  When 他打開清單
  Then 那一台的標示與一般的已停止不同
  And 畫面上出現「某一支策略腳本找不到了」這個原因
...
**As a** 使用者，**I want** 在畫面上選策略腳本、取代號、拼條件，
**so that** 「甲說買、乙說賣的時候該怎麼辦」這個判斷被我寫下來,而不是每次重想。

```gherkin
Scenario: 填完三段就存得起來
  Given 他的可用策略腳本裡有策略腳本甲
  When 他填上名稱、交易標的與觸發間隔
  And 他加入一個信號來源:策略腳本甲、一小時刻度、代號 A
  And 他把買入條件設為「A 等於買入」
  And 他把賣出條件設為「A 等於賣出」
  And 他按下儲存
  Then 那一台機器人出現在清單上
...
Scenario: 信號來源只挑得到自己的可用策略腳本
  Given 他的可用策略腳本裡有策略腳本甲與策略腳本乙
  When 他要為一個信號來源挑策略腳本
  Then 選單裡有策略腳本甲與策略腳本乙
  And 選單裡沒有任何別人未發佈的策略腳本

Scenario: 只挑得到會吐訊號的那幾支
  Given 他的可用策略腳本裡有一支訊號種類的,以及一支吐數字的
  When 他要為一個信號來源挑策略腳本
  Then 選單裡只有那一支訊號種類的
  And 選單裡沒有那一支吐數字的

Scenario: 同一支策略腳本加兩次,只要代號不同
  Given 策略腳本甲宣告了一個參數「回看根數」
  When 他加入代號 A 的來源:策略腳本甲、回看根數 20
  And 他加入代號 B 的來源:策略腳本甲、回看根數 60
  And 他按下儲存
  Then 儲存成功
  And 那一台機器人有兩個信號來源
...
Scenario: 指名一支已經不在的策略腳本由後端說
  Given 他挑的那支策略腳本在送出前被刪掉了
  When 他按下儲存
  Then 畫面顯示後端說的「找不到」
  And 他填的內容還在表單上
...
- **擋得住的擋在畫面，擋不住的照後端說的講。**
  ...
  擋不住 ＝ 需要知道伺服器那邊有什麼（撞名、策略腳本還在不在）。
- **只有訊號種類的策略腳本挑得到。** 機器人跑一個信號來源時**一律以訊號種類執行它**，
  所以一支吐數字的策略腳本會在第一輪算式失敗——而算式失敗是會**停擺**的那一類。
...
- **一支會吐信號的策略腳本都沒有**：說得出要先寫一支，而不是給一個空的下拉選單。
  注意這與「一支策略腳本都沒有」不同——他可能有五支，只是沒有一支是訊號種類的。
...
- **External Dependencies:**
  ...
  - 既有的**可用策略腳本**清單（挑策略腳本與讀它宣告的參數都靠它）。
