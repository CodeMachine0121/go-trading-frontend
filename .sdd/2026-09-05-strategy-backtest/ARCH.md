...
指導原則：**工作區不屬於任何一個去處，所以它擺在切換之上。**

這一句同時解決了前兩題。工作區（算式、策略、參數、市場、粗細）留在
`IndicatorCalculationPanel` 手上，兩個去處都只是它下面的一塊；
...
│  策略列（挑策略／存／另存／改名／清單）    ← 共用   │
│  工作區（算式編輯器 + 種類 + 參數 + 說明） ← 共用   │
│  ┌ AppTabs ──────────────────────────┐             │
│  │  指標預覽  │  回測                │             │
│  └───────────────────────────────────┘             │
│  ┌ v-show=預覽 ─────────┐ ┌ v-show=回測 ─────────┐ │
│  │ 執行條件（標的/多長/ │ │ StrategyBacktestPane │ │
│  │ 粗細）→ 提示 → 結果  │ │                      │ │
│  └──────────────────────┘ └──────────────────────┘ │
└────────────────────────────────────────────────────┘
```
...
另一邊就得跟它借；放進切換之下的共用區塊，切換時它仍會被重新安排。
放在上面，它與去處完全無關——而 CodeMirror 編輯器因此一次都不會重新掛載。
...
| 每筆賺賠（紅） | `'-120'` 與 `'negative'` | 一個 `Decimal` 讓元件自己比大小 |

**金額例外於「直接可畫」**：資金曲線的每一點帶的是 `Decimal`，
到繪圖那一刻才 `.toNumber()`——與 K 線圖對價格的做法一致。
繪圖函式庫只吃數字，那是繪圖的限制，不是可以在別處把金額變成浮點數的許可。
...
| 檔案 | 改什麼 | 為什麼必須 |
| :--- | :--- | :--- |
| `IndicatorCalculationPanel.vue` | 加去處切換；既有的預覽內容原封包進 `v-show`；掛上回測去處；載入策略時把 `workspaceGeneration` 加一 | §2、§3 |
