...
- **BR-5 就地把關的四條**：起點不得晚於終點；本金必須是大於零的數；百分比必須大於零且不超過一百；固定金額必須大於零。另加算式不得空白（沿用指標預覽既有那一條，不另立一套）。**說明一律留在出問題的那一格旁邊。**
...
- **版面**：策略那一列 →**共用的工作區**（算式編輯器與它的旋鈕）→ **去處切換**（指標預覽｜回測）→ 該去處自己的執行條件、狀態與結果。
  工作區擺在切換之上，是因為它是兩個去處共用的那一份——擺在切換之下就得選一邊，而它不屬於任何一邊。這也讓**切換去處時編輯器完全不受影響**，這正是本切片最重要的那一條要求。
...
| 兩個去處怎麼擺 | **共用的工作區擺在切換之上，各自的執行條件與結果擺在切換之下** | 工作區不屬於任何一邊，擺在切換之下就得選一邊；擺在上面同時保證了切換去處時編輯器不會重新掛載 |
