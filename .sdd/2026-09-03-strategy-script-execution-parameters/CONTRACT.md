# 策略腳本與執行參數分離（操作介面） — Contract Verification
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| AC-01.1 | 另存為新策略腳本時只存下算法 | O-01：送出的內容只有名稱、算式、種類 | `strategy-script-write-domain.ts:19`、`strategy-script-proxy.ts` 的 body | `strategy-script-proxy.spec.ts:打的是建立策略腳本的端點`（body 完整比對） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-01.2 | 載入一支策略腳本帶進來的只有算法 | O-02：算式與種類換成該策略腳本的 | `strategy-script-domain.ts:20` | `IndicatorCalculationPanelStrategyScript.spec.ts:挑一支就把它記住的算法帶進畫面` | asserts-oracle | produces-oracle | ✅ conforms |
| AC-01.3 | 讀回一支策略腳本時沒有取數計畫可讀 | O-03：形狀含名稱／算式／種類，**不含**刻度與根數 | `strategy-script-content-dto.ts:19`、`strategyScript.ts:14` | `strategy-script-application.spec.ts:讀回來的策略腳本身上沒有取數計畫可讀`（**本次稽核補上**） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-02.1 | 載入策略腳本不動彙總刻度與計算根數 | O-04：載入前 1h／60 → 載入後仍是 1h／60 | `IndicatorCalculationPanel.vue` 的 `applyContent` 只寫兩個 ref | `IndicatorCalculationPanelStrategyScript.spec.ts:挑一支不會動到這一次的執行設定` | asserts-oracle | produces-oracle | ✅ conforms |
| AC-02.2 | 載入策略腳本不動交易標的 | O-05：載入前後交易標的相同 | 同上（交易標的本來就不在策略腳本內容裡） | 同檔:`挑一支不會動到交易標的——策略腳本不記交易標的`（既有） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-02.3 | 開一份新的空白策略腳本也不動執行設定 | O-06：算式清空、種類回預設；4h／60 不變 | `blankStrategyScriptContent` 只有兩欄 | 同檔:`開一份新的空白也不動這一次的執行設定` | asserts-oracle | produces-oracle | ✅ conforms |
| AC-03.1 | 改動彙總刻度不算未儲存的變更 | O-07：不出現確認，直接載入 | `strategy-script-draft-domain.ts:22` 只比兩欄 | 同檔:`只改了彙總刻度時不問`（`it.each` 第一列） | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-03.4 | 改動指標值種類算未儲存的變更 | O-10：出現確認 | 同上 | `strategy-script-draft-domain.spec.ts:指標值種類 改了就算有未儲存的變更` | asserts-oracle | produces-oracle | ✅ conforms |
| AC-03.5 | 還沒載入過任何策略腳本且算式空白時不問 | O-11：直接載入，不出現確認 | `strategy-script-draft-domain.ts:20`（既有規則） | `IndicatorCalculationPanelStrategyScript.spec.ts:編輯區還沒動過時直接帶入，不多問` | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-04.4 | 那句道歉的話不再出現 | O-15：畫面文字不含「目前計算仍以五分鐘執行」 | `IndicatorCalculationPanel.vue` 的欄位說明 | `IndicatorCalculationPanelStrategyScript.spec.ts:選單在，且不再說它還沒生效`（`not.toContain`） | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-07.1 | 存回使用中的那一支 | O-23：存回並顯示已儲存訊息 | 未改動 | `IndicatorCalculationPanelStrategyScript.spec.ts:有使用中的那一支時，儲存存回它而不是建立新的` | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-07.4 | 載入一支認不出外框的策略腳本 | O-26：整段原文帶入並告知 | 未改動 | `IndicatorCalculationPanelStrategyScript.spec.ts:認不出外框的算式整段帶進來，並說出這一支不是在這裡寫出來的` | asserts-oracle | produces-oracle | ✅ conforms |

### Section 4 的核心規則（BR）

| ID | Rule | 判定 |
| :--- | :--- | :--- |
| BR-1 | 策略腳本記住三樣 | 由 AC-01.1／01.3 覆蓋 ✅ |
| BR-2 | 執行設定三樣，不隨策略腳本存下來、不隨載入被覆蓋 | 由 AC-01.1／02.1／02.3 覆蓋 ✅ |
...
未加「算到哪個時間為止」、未在圖表上畫任何東西、未替既有策略腳本補回那兩樣、
...
- **AC-01.3**（讀回策略腳本時沒有取數計畫可讀）原本是 🟡 **partial**：
  程式碼正確，但那兩樣的「不存在」**只有型別系統擋著**——那是建置時的保證，
  沒有任何測試在執行期釘住它。補上
  `strategy-script-application.spec.ts:讀回來的策略腳本身上沒有取數計畫可讀`，
  並以突變確認：把彙總刻度加回策略腳本內容，該測試立刻紅。
