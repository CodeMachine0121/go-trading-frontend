# 策略腳本與執行參數分離（操作介面） — Architecture Design
...
- **In one sentence:**
  把彙總刻度與計算根數從 `StrategyScriptContentDto`（策略腳本記著的東西）搬到
  `IndicatorCalculationRequestDto`（這一次執行的東西），並讓結果帶回系統回報的刻度。

- **Guiding principle:**
  **讓 `StrategyScriptContentDto` 縮小，其餘的行為自己就對了。**
  ...
  而管理未儲存狀態的 `useStrategyScriptLibrary` 一行都不必改。

  若改成保留四欄、再在三處各加一個「這兩欄不算」的例外，就會有三個各自會漂移的地方，
  而漏掉任何一個的徵狀是靜靜蓋掉使用者寫的東西。

- **第二原則：一個刻度字串該怎麼被讀，只能有一份。**
  目前這條規則唯一的家是 `StrategyScriptDomain` 裡的私有方法 `aggregationInterval()`——
  它把系統給的字串對上認得的那五種，認不得就退回最細的。
  本切片把它從策略腳本身上拿掉，但這條規則現在有**兩個**需要它的地方
...
| `domain/models/dto/strategy-script-content-dto.ts` | **Modify** | **本切片的支點**：四欄縮成兩欄（算式內容、指標值種類） |
| `domain/models/entities/strategy-script.ts` | **Modify** | 移除 `aggregationInterval`、`candleCount` |
| `domain/models/domains/strategy-script-domain.ts` | **Modify** | 移除私有的 `aggregationInterval()`——規則搬進新的 Domain Model，且策略腳本不再需要它 |
| `domain/models/domains/strategy-script-write-domain.ts` | **Modify** | 不再攜帶那兩樣 |
| `domain/models/domains/strategy-script-draft-domain.ts` | **Modify** | 少比兩欄。**不是順手簡化**——多比一欄就會為不屬於策略腳本的東西跳確認 |
| `infrastructure/proxy/strategy-script-proxy.ts` | **Modify** | wire 型別與送出的 body 各去掉兩個欄位 |
| `domain/service/strategy-script-service.ts`／`application/strategy-script-application.ts` | **Modify** | 移除 `listAggregationIntervalOptions` / `defaultAggregationInterval` / `defaultCandleCount`——它們描述的是一次計算，不是一支策略腳本 |
...
| `components/organisms/IndicatorCalculationPanel.vue` | **Modify** | 兩個欄位改由自己的 ref 持有（不再進出策略腳本內容）；兩句說明改寫；結果列多一項 |
| `composables/use-strategy-script-library.ts` | **Not touched** | 它只認識 `StrategyScriptContentDto`；那個形狀變小了，它一行都不必改。**這是支點選對了的證據** |
...
| :--- | :--- | :--- |
| `StrategyScriptContentDto` | 策略腳本記著的四樣，同時是載入、儲存與比對的唯一形狀 | 剩兩樣。US-01／US-02／US-03 三組 AC **全部由它縮小推導出來** |
| `StrategyScriptDraftDomain` | 比對載入當下與現在 | 少比兩欄——不是簡化，是把比對範圍收回策略腳本真正記著的範圍 |
...
| `IndicatorCalculationService` | 指標計算的編排 | 接手三個「一次計算的預設與選項」方法。搬家的判準是「這個問題屬於誰」：一次計算要吃多粗，不是一支策略腳本的事 |
| `IndicatorCalculationPanel.vue` | 指標計算這一整塊 | 兩個 ref 不再進出 `StrategyScriptContentDto`；刻度選項改問指標計算那一側；兩句過期的說明改寫；結果列多一項 |
...
flowchart TD
    ...
    Panel --> SApp[StrategyScriptApplication]
    Panel --> Lib[useStrategyScriptLibrary（不改）]
    Lib --> SApp
    ICApp --> ICSvc[IndicatorCalculationService]
    SApp --> SSvc[StrategyScriptService]
    ...
    SSvc --> SProxy[(IStrategyScriptProxy)]
```
...
- **Most likely next requirement:** 在 K 線圖表上套用一支策略腳本、把值畫出來。
  （`2026-09-03-strategy-script-library` 的 ARCH 已經預言過這一步，只是當時說的是「一鍵執行」。）
- **Where it lands:**
...
- **Known debt / deferred:**
  ...
  - 彙總刻度真的生效使同一支策略腳本以相同根數執行會得到與先前不同的數字。
    靠結果列上寫出刻度讓它看得見，不另做遷移提示。
  - **指標計算那一側有七個「問我要一個常數」的方法**（兩個預設值、三份清單、
    一份算式樣板、一份欄位說明），面板要串六個呼叫才組得出初始狀態。
    看起來像可以收成一個「一次空白計算長什麼樣」的描述物件——**刻意不做**：
    那個物件會把「渲染選單要用的清單」與「初始值」綁在同一個信封裡，
    兩者會因為完全不同的理由改變，收在一起只是換一個更難拆的形狀。
    **回頭處理的訊號**：這組方法再多兩個，或出現第二個呼叫端也得串同一串。
  - **`AggregationIntervalDomain` 與 `IndicatorResultTypeDomain` 結構幾乎相同**
    （讀一個宣告字串 → 正規化 → 給名字 → 交出選項）。**刻意不抽共用基底**：
    兩個相似的東西不是重複，而且它們已經開始分歧（種類還要回答「是不是一串」
    「裝的是不是數字」）。對兩個實例做的抽象，通常在第三個出現時才發現抽錯了。
    **回頭處理的訊號**：出現第三個同形狀的模型。
...
| :--- | :--- |
| US-01 另存為新策略腳本時只存下算法 | `StrategyScriptContentDto` 縮小 + `StrategyScriptWriteDomain` + `StrategyScriptProxy` 的 body |
| US-01 載入一支策略腳本帶進來的只有算法 | `StrategyScriptDomain.toDto` |
| US-01 讀回一支策略腳本時沒有取數計畫可讀 | `StrategyScript`（entity）+ `StrategyScriptProxy` 的 wire 型別 |
| US-02 載入策略腳本不動彙總刻度與計算根數 | `IndicatorCalculationPanel.vue` 的 `applyContent` 只寫兩個 ref |
| US-02 載入策略腳本不動交易標的 | 既有行為（不改） |
| US-02 開一份新的空白策略腳本也不動執行設定 | 同上（`blankStrategyScriptContent` 也只有兩欄） |
| US-03 改動彙總刻度／計算根數不算未儲存的變更 | `StrategyScriptDraftDomain` 只比兩欄 |
| US-03 改動算式內容／指標值種類算未儲存的變更 | 同上（保留的那兩欄） |
| US-03 還沒載入過任何策略腳本且算式空白時不問 | `StrategyScriptDraftDomain` 既有規則（不改） |
...
- **Risks / trade-offs:**
  - **依賴後端先部署。** 舊的系統端會拒絕少了兩個欄位的策略腳本請求形狀。個人專案、兩邊一起更新，可接受。
