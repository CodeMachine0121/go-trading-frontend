...
- **Out of Scope:** 下單與交易執行、即時推播（無 SSE／WebSocket）、多使用者與權限、
  K 線圖表繪製（第一階段以數字表格為準）、任何前端自有的資料庫或持久化。
  **行動裝置不再列於此**：每一個畫面都要在手機直立（寬 390）上可讀可用，見下方 Constraints。
...
- **Screen Constraints:** 最窄支援**寬 390**（手機直立）；手機橫放時可用高度約 390 亦須成立。
  兩道分界：**768** 決定版面的疏密、積木工作檯能不能編、K 線控制項起始收不收、助手蓋不蓋滿畫面；
  **1024** 決定導覽是抽屜還是固定側欄。這兩個數字在樣式與程式各有一份副本，兩處互指，不得出現第三份。
...
| :--- | :--- |
| K 線 | 市場在固定一分鐘內的價量摘要，以「交易標的 + 起始時間」唯一辨識 |
| 交易標的 | K 線所描述的市場，例如 BTCUSDT；查詢與計算都必須指定 |
| 起始時間 | K 線涵蓋的一分鐘從何時開始；必須落在一分鐘刻度上、不得指向未來、一律 UTC |
