...
### 12.3 補測試時抓到的第三個真 bug：焦點還不回來

PRD 有一條「回答回來後解鎖並聚焦」，原本沒有測試。補上之後它紅了。

原因是時機：等待中輸入框是**鎖住的**，而 `watch` 的預設時機在畫面更新之前——
對一個還鎖著的輸入框叫 `focus()` 什麼都不會發生。於是每問一句都得重新點一次輸入框，
而那種不順手沒有人會回報成 bug，只會覺得這個東西有點難用。

改成 `flush: 'post'`（等畫面更新完才做）。

---

## 13. 抽屜的寬度可以拉（第五輪回饋後補記）

回饋：抽屜要能讓使用者自己調寬度。

和那顆鍵的位置是同一類東西——**這台裝置的習慣**——所以走同一條路，
一層都沒有跳過：

| 層 | 新增 |
| :--- | :--- |
| Domain Model | `AssistantDrawerWidthDomain`——**夾回一個還能用的寬度**這條規則 |
| Interface | `IAssistantDrawerWidthPreferenceProxy` |
| Proxy | `AssistantDrawerWidthPreferenceProxy`（瀏覽器儲存；讀不出來＝沒拉過） |
| Service / Application | `AssistantDrawerWidthService` / `AssistantDrawerWidthApplication` |
| Composable | `useAssistantDrawerWidth`（拉動編排；pointer 事件掛 window） |
| Organism | `AssistantDrawer` 多一個 `width` prop 與左緣那條把手 |

**沒有 DTO。** 一個寬度就是一個數字，包一層只是儀式——
`selectConversation(id: number)` 早就這樣傳了。

### 13.1 那條規則為什麼值得進 domain

三個界線各有各的理由，而且三個都不是「看起來比較好」：

- **最窄 320**——再窄它就不能用了：帶小標與條列的回答會變成一行一兩個字，
  輸入框與送出鍵擠成一團。使用者當然可以把它拖窄，但不能拖到失能。
- **最寬 720**——它是**疊在畫面上暫時叫出來的東西**，不是第二個頁面。
  寬到蓋掉大半個視窗，使用者就看不到自己正在問的那張圖了，
  而「看著圖順手問一句」是它存在的理由。
- **跟著視窗收**——上次在寬螢幕拉到 700 的人，這次開一個窄視窗時，
  不該看到一塊比視窗還寬的面板。

因此這條規則不只在拉動時套用：讀回記住的寬度、視窗大小改變時也要套。

### 13.2 為什麼是新的 service 而不是塞進 `AssistantTriggerService`

兩者都是「這台裝置對助手的習慣」，但會分開改變：一個是抽屜多寬，
一個是那顆鍵的位置與拖曳門檻。合成一個 service，它的公開方法會乾淨地分成
兩半互不相干——那正是一個盒子裡裝了兩樣東西的樣子。

### 13.3 幾個實作細節

- **往左拉是變寬。** 抽屜靠右，所以會動的是左緣：`寬度 = 起始寬度 + (起始游標 − 現在游標)`。
- **寬度從 prop 進來、畫在行內樣式**，與那顆鍵的大小同一個理由：
  夾回範圍的算式與畫出來的必須是同一個數字。SCSS 裡原本的 `$drawer-width` 因此刪掉，
  只留 `max-width` 那一行當極窄視窗的退路。
- **只是點了一下那條邊就不記住**（寬度沒變還寫一次瀏覽器儲存是白寫的）。
- 事件掛 window，因為手一快就會離開那條五像素的細邊。
- 共用狀態裡放的仍然是**一個數字**——§10.4 那個 SSR 的教訓。

### 13.4 驗證

新增的五支（domain model、proxy、service、application、composable）
覆蓋率都是 **100%**（statements 與 branch）。變異 **8/8 全殺**，
包含把拉動方向弄反、拿掉最寬上限、極窄視窗夾出比最窄更小的值、
沒抓住就移動也開始拉、把寬度畫死。

---

## 14. 回答裡的程式碼要有行號與著色（第六輪回饋後補記）

回饋：助手回答裡的程式碼區段，能不能長得像算式編輯器那樣有語法著色與行號——
**但訊息不可以被修改**。

### 14.1 不做第二個著色器，用同一個元件的唯讀樣子

`AppCodeEditor` 早就有 `readonly`（README 說過「唯讀外框與可編輯內容是同一個原子的
兩種樣子」），所以這裡什麼都不必發明：直接用它。

這也正是規範那條「一個 UI 概念只有一個元件」的用處——
使用者會把助手給的那段東西**貼進算式編輯器**，兩邊長得一樣他才認得出那是同一種東西；
而後端說「第 12 行出錯」時，畫面上就是那一行。

**唯讀不是樣式上的收斂**，是那個元件的一種樣子：它拿掉整組編輯用的擴充
（歷史、補齊、快捷鍵、縮排），並讓編輯器自己回報 `EditorState.readOnly` 與
`EditorView.editable.of(false)`。測試守住兩件事：那塊 DOM 的 `contenteditable`
是 `false`，而且**就算有人在那塊 DOM 上敲字，也沒有一條把它寫回去的路**。

### 14.2 `code` 與 `preformatted` 分家

原本兩者都是 `preformatted`（§ ARCH 3.2 那時只有五種塊）。現在分開，因為它們是兩種東西：

- **`code`** 是助手用圍欄圈起來的一段程式碼。它值得行號與著色。
- **`preformatted`** 是助手排的表格那幾行。它照原樣當文字——那不是程式碼，
  給它行號只會讓人以為那是可以貼去執行的東西。

拆解器因此多記一件事：圍欄上宣告的**語言**。它**只用來標給人看**，不用來挑著色器——
這個操作台的程式碼區塊是 Go 的那一個，而它只認得 Go。標註是那份著色的誠實對照：
助手貼一段 JSON 進來時，顏色是 Go 味的，但右上角那個小字說得出那其實是 JSON。

要真的支援第二種語言，是在 `AppCodeEditor` 裡加，不是在這裡加。

### 14.3 變異測試逼出來的三件事

這一輪跑了 11 個變異，前後抓到**兩個等價變異**與**兩個真漏洞**：

- **兩個等價變異**（開圍欄時多設一次種類、關圍欄時清語言）——它們改掉之後行為完全一樣，
  也就是那兩行是多餘的。對等價變異的正確反應是**把多餘的程式碼刪掉**，
  不是為一個不存在的差異寫測試。刪完之後圍欄那一段也好讀了：開就記語言、關就是關。
- **兩個真漏洞**：`圍欄關起來之後的文字`（少了「關起來」，後面整段話會被吞進程式碼裡）
  與 `兩段程式碼背靠背`（中間連空白行都沒有時，兩塊會黏成一塊、還掛上後面那一塊的語言）。
  兩者都補了測試。

### 14.4 驗證

拆解器與 `AssistantAnswerBlocks` 的 statements 都是 **100%**。
變異最終 **11/11 全殺**（含把 `readonly` 拿掉、把表格當成程式碼、丟掉圍欄上的語言）。

---

## 15. 追加一輪：複製鍵（超出原 ARCH）

使用者的話：「chatbot 聊天視窗，是不是可以加一個 copy 的按鈕呢」。

### 15.1 兩個位置，因為那是兩件事

查過的設計慣例（Mobbin：Cohere、Grok 的程式碼區塊；Gemini 的訊息動作列）裡兩種都有，
而它們回答的是不同的需求，所以**兩個都做**：

| 位置 | 複製什麼 | 為什麼 |
| :--- | :--- | :--- |
| 程式碼區塊右上角 | 那一段程式碼本身 | 那段東西是要貼進算式編輯器的；圍欄的三個反引號不該跟去 |
| 回答底下的動作列 | 那一則回答的**原文** | 整段留存／貼去別處；拆好的塊是給眼睛看的 |

提問那一側**沒有**複製鍵——使用者剛剛才打完那句話。

### 15.2 剪貼簿是外部資源，所以它走完整的一條路

`navigator.clipboard` 是瀏覽器提供的外部能力，依 `.claude/rules/naming.md`
一律以 `Proxy` 封裝，元件不得直接碰：

```
CopyTextButton → useCopyText → ClipboardApplication → ClipboardService → IClipboardProxy
                                                                          └ ClipboardProxy（navigator.clipboard）
```

service 裡只有**一條真規則**：**空的一段永不碰剪貼簿**。它不是防禦性檢查——
一次成功的空複製會把使用者原本剪貼簿裡（他還要用的）東西清掉。
判斷「有沒有東西」看的是去掉空白之後，但**複製的是原樣**：一段程式碼開頭的縮排是它的一部分。

`ClipboardWriteFailedError` 是這條邊界的哨兵錯誤。瀏覽器在非安全連線、權限被拒、
或這次點擊沒被算成使用者動作時都會拒絕——對使用者是同一件事，而**靜靜失敗**是最壞的結局。

### 15.3 狀態是每一顆各自的

`useCopyText` 的 `ref` 是**每次呼叫各自一份**，不是 `useState`：
共用一份的話，按了某一段程式碼旁邊那顆，畫面上每一顆都會同時打勾，
使用者會不知道自己到底複製了哪一段。那個勾兩秒後自己退回去
（一顆永遠打著勾的鍵，下一次按下去就看不出有沒有反應），連按兩下則從後面那一下重新算。
元件收掉時清掉計時器——抽屜關掉的瞬間按過複製就正好踩到那裡。

### 15.4 原文得多帶一份

`ConversationMessageDto` 因此多帶一個 `content`（原本只有拆好的 `blocks`）。
**拆好的塊是給眼睛看的，這一份是給剪貼簿用的**——從塊反推原文會掉圍欄、掉語言、掉空行。

### 15.5 驗證

剪貼簿那一條路（proxy／service／application）、`useCopyText`、`CopyTextButton`
的 statements 都是 **100%**；全站 1410 個測試綠燈。
變異 **16/16 全殺**，其中三個活過第一輪：

- **「失敗那句話不留」** 活著是因為測試用的錯誤訊息剛好與程式裡的罐頭話**同一句**，
  那一條分不出有沒有在聽。換成一句不同的話才真的守住。
- **「圖示不換」** 活著是真漏洞：那顆鍵很小，多數人看的是圖不是字，而沒人守著它。
- 第三個順路揭出一件**與複製無關**的事：`v-if="message.role === 'answer'"`
  在 `AssistantMessage` 裡出現兩次，改到的是**機器人頭像**那一條——
  而「頭像只掛在回答那一側」原本沒有測試。補上了。
