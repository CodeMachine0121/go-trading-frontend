...
| R13a | 挑了我加入的「別人的」，試跑時系統回「算式出錯：第 3 行出錯」 | 按試跑 | 執行結果那裡顯示含「第 3 行出錯」的算式問題說明 |
| R13b | 挑了我加入的「均線交叉」（種類「一個信號」） | 切到回測、執行 | 送去回測的請求指名 9、不帶算式；顯示「回測了 12 根」的成績單 |
...
| W12 | 連線燈的檢查 `fetchBackendHealth`（打開時一次、按重新檢查時一次；**不定期**） | `beginWaiting` 被叫一次、結束一次（由 contract 回饋修正：它是使用者按了之後在等的） |
