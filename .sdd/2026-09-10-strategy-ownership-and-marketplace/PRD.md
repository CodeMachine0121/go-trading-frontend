...
**As a** 逛市集的人，**I want** 打幾個字就把眼前縮到剩下幾張，**so that** 我不必為了一支已經知道名字的策略往下捲三十張。
...
Scenario: 沒寫說明不影響其他欄位
  Given 市集上有一支沒有寫說明、名稱是「布林通道」的策略
  When 在搜尋的地方打「布林」
  Then 看到那一支
...
11. **搜尋只決定看得到哪幾張。** 不改順序、不改任何一支策略、不影響加入與移除，也不留存。
    而「沒有符合」與「市集上還沒有任何策略」永遠分開講——第 8 條的同一個道理：
