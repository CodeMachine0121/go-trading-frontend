...
0. **Code review 之後補修了五個地方，其中兩個會讓畫面卡住或把人送錯地方。**

   | 問題 | 後果 |
   | :--- | :--- |
   | 「已經確認過」是一個布林，而且失敗時也留下 | 後端還沒啟動時打開操作台，這個分頁就**永遠回不去**——後端起來了也一樣，只能整頁重新載入 |
   | 那個布林在動作**開始**時就設起來 | 答案還沒回來時問的人會拿到一個很肯定的「沒登入」，被帶到登入畫面；真正的答案回來時已經沒有人在等它 |
   | 把關拿 `to.path` 直接跟 `/login` 比字串 | 走 `/Login` 或 `/login/` 時，**登入成功之後會被送回登入畫面**，讀起來像登入失敗 |
   | `signOut` 沒有進行中的防護 | 連按兩下會把**同一份**續用憑證送去撤銷兩次，而後端把「同一份出現兩次」讀成盜用 |
   | `pending` 在換頁之前就放開 | 換頁還在進行時畫面仍是登入卡片而那顆鍵已能按，按 Enter 會開出第二段登入階段，第一段沒有人撤得掉 |

   前兩個由「存下進行中的那個動作，而不是一個問過了沒的布林」一起解決。
   每一個修正都補了測試並做過 mutation 檢查（前三次 mutation 活下來——**是測試不夠力，不是程式沒問題**——
   已把那三則測試改寫成抓得到才算數）。

1. **已知但刻意不改的一個取捨：換發的回應遺失時，下一次打開會誤觸後端的盜用偵測。**

   換發送出去了、後端也換了，但回應沒回到瀏覽器（重新整理、網路斷、電腦睡著）——
   本機留著的就是一份已經被用掉的續用憑證，下一次打開會把它再送一次。

   **另一種做法更糟**：「換發一失敗就把記著的東西丟掉」會讓「後端只是還沒啟動」
   這個常見得多的情況直接把人登出。真正的解法在後端（給剛換發過的那一份幾秒寬限期），
   已記在 PRD §7 與 ARCH §5 的訊號裡。
