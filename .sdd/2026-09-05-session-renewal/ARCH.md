...
flowchart TD
    ...
    G -- 連不上 --> X
    K -- 已經換過 --> D
    K -- 還沒換過 --> I[renewSession → writeSession]
    I -- 被拒絕 --> D
    I -- 成功 --> J[再問一次] --> H
    J -- 再被拒絕 --> D
```
...
- **刻意留簡單的：**
  ...
  - **換發送出去了但回應沒回來時，本機留著的是一份已經被用掉的續用憑證**，
    下一次打開會把它再送一次而誤觸盜用偵測。
    保留這個行為是因為另一種做法（換發一失敗就清掉）會讓「後端只是還沒啟動」
    這個常見得多的情況直接把人登出。**真正的解法在後端**：給剛換發過的那一份
    幾秒鐘的寬限期，在那段時間內重放只回絕、不撤鏈。**訊號**是使用者抱怨
    「重新整理一下就被登出」。
