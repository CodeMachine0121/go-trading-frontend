...
- **Expected Outcome:**
  種類選單多「一個信號」；選它時算式外框回傳一個信號、範例示範怎麼選；指標預覽算出信號時畫面呈現一個「買入／賣出／持有」的結論（沒有指標名稱）；回測改成擋「不是信號種類」並把訊息改寫；過時的信號說明換成「算式回傳 buy／sell／hold」；信號比照是非，在圖上畫不成線、挑策略腳本時擋下。
...
- **Primary Role(s):** 策略腳本研究者（專案作者本人）——寫算式、調參數、判斷一支策略腳本值不值得留下來。
...
**As a** 策略腳本研究者, **I want** 種類選單多一個「一個信號」,
...
**As a** 策略腳本研究者, **I want** 選「一個信號」時外框就換成回傳一個信號的形狀,
...
**As a** 策略腳本研究者, **I want** 信號種類算出來時畫面直接給我一個結論,
...
**As a** 策略腳本研究者, **I want** 回測在種類不對時擋下並告訴我改哪裡,
...
**As a** 策略腳本研究者, **I want** 回測規則與信號讀法的說明講的是現在的事實,
...
**As a** 策略腳本研究者, **I want** 挑一支信號種類的策略腳本要畫到圖上時被擋下,
**so that** 我不會套用之後才發現畫不出東西。

```gherkin
Scenario: 信號種類的策略腳本標明畫不成線
  Given 一支策略腳本的指標值種類是「一個信號」
  When 使用者在挑策略腳本的清單看它
  Then 標明畫不成線，套用被擋下
  And 呈現方式與「一個是非」的策略腳本一致
```
...
- **Core Business Rules:**
  ...
  - **BR-8 信號畫不成線**：「一個信號」比照「一個是非」——挑策略腳本時標明畫不成線並擋下，畫指標線時不畫。
