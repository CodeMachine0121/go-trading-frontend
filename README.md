...
| `/connector-authorization?request=…` | **授權外掛** | Claude Code 這類外掛要以你的身分使用交易服務時，把瀏覽器送到這裡。說清楚是哪一個外掛、你目前登入的帳號、允許後它能使用**全部功能**；**只有你親手按「允許」才算數**，按「允許」或「拒絕」後瀏覽器被送回外掛，這一頁說可以關掉分頁。授權請求已失效（十分鐘、或已被用過）時請你回 Claude Code 重新連線。要先登入且已開通；沒登入時先到 `/login`，登入後回到同一張授權請求。**不套用操作台外框** |
...
**助手改你既有的東西要你按確認。** 後端不讓助手直接改使用者既有的策略腳本與交易策略，
只留下一筆**待確認修改**；它列在那一次問答最後一則訊息的下方（`AssistantPendingRevisionCard`），
說得出改的是哪一支、狀態，展開看得到改成的完整內容，就地確認或拒絕。
按下之後**整段對話重讀**，狀態只信後端；被擋下（執行中的機器人在用、提出後已被改過…）時
後端那一句留在那一筆底下，那一筆維持原樣可以再按。認不出來的狀態不給鍵——
寧可少一顆鍵，也不讓人對一筆不確定的東西按確認。
...
- **被門擋下來的人，登入之後會回到他本來要去的那一頁**，不是第一站（K 線圖表）——連同網址上的查詢參數，所以外掛授權同意頁登入後仍認得是哪一張授權請求。
