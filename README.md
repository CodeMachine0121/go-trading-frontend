...
| `/chat` | **行情助手** | 用日常講話的方式問行情，助手自己去查交易標的、K 線、指標與策略再用一段話回答。左邊是對話清單、右邊是對話串；**任何畫面都能從右下那顆鍵叫出同一段對話的抽屜**，抽屜標頭的展開再帶回這一頁 |
...
- **抽屜**給「看著圖順手問一句」用：右側 420 像素的單欄，畫面其餘部分照樣看得到。
  它**不放對話清單**——窄欄硬塞兩欄的結果是兩邊都難用，而換對話是低頻動作，
  標頭的展開帶著同一段對話跳到整頁去換。
...
Claude 的額度警示塊；WRITER、Origin、Cohere 的建議提問）。
