...
| `/contract-strategy-scripts` | **合約策略腳本** | 同一塊工作區，寫的是吃**合約行情格**的算式：每一格除了成交價，還帶成交筆數、標記／指數／溢價指數三組開高低收、收盤時現行的資金費率、這一格內有沒有結算與持倉統計（沒有值一律為零）。標的從合約標的清單挑；清單只列合約行情種類；**合約回測尚未開放**，所以沒有回測分頁 |
