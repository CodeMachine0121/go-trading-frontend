...
| `/watchlist` | **觀察清單** | 後端要持續追蹤哪幾檔。加進來之前後端會先向該市場確認代號存在——**打錯字當場被擋下**，而不是換來一條每輪安靜失敗的線；加不成功時說得出是「你打錯了」還是「等一下再試」，兩者的下一步完全相反。**移除只停止追蹤**，已經抓回來的 K 線一根都不刪，所以確認訊息會把這件事說出來。改動最多等一輪（五分鐘）生效，不必重開後端 |
