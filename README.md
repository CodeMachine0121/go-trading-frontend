...
| 路徑 | 做什麼 | 重點 |
| :--- | :--- | :--- |
| `/login` | **登入** | 沒登入的人**唯一**看得到的畫面。同一張卡片上有登入與建立帳號兩種模式，切換不換頁、已填的內容留著。**不套用操作台外框**——側欄與頂欄是給進了門的人看的 |
| `/` | **（轉到第一站）** | 不是一個畫面：走到這裡的人一律被帶到 K 線圖表。後端是否可用由側欄底下那顆燈說，走到任何畫面都亮著 |
| `/k-candles` | **K 線瀏覽與維護** | 指定標的與區間查詢，結果**由新到舊**列出並標示漲跌；每一列可編輯，另可新增與刪除（刪除需二次確認），任何一次成功的維護都會自動重查 |
| `/k-candles/chart` | **K 線圖表** | 同一批 K 線改用圖呈現。**會收盤的市場（台股）多一顆「立刻更新」**，按一下把缺的 K 線補回來、重畫一次。**拉遠拉近就是在選要看多長**，每根涵蓋多久跟著自動換（五分鐘／十五分鐘／一小時／四小時／一天，挑最細的且畫面上不超過 400 根的那一種）；快捷區間一天到一年一鍵切換，畫法可在蠟燭與曲線之間切換。取資料時兩側各多取半段，因此小幅拖動不重新取。**「看什麼」那塊控制項收得起來**，收起來的高度全部歸圖；套在圖上的每一支策略腳本各有一顆眼睛，**按一下把它的線收起來但不拿掉它**（照樣跟著重算、顏色照樣佔著，再按一下就是同一條線），**收著的那幾筆這台瀏覽器記著**，明天打開仍然收著 |
| `/chat` | **行情助手** | 用日常講話的方式問行情，助手自己去查交易標的、K 線、指標與策略腳本再用一段話回答。左邊是對話清單、右邊是對話串；**任何畫面都能從那顆圓形的機器人鍵叫出同一段對話的抽屜**（那顆鍵可以拖到你要的位置），抽屜標頭的展開再帶回這一頁 |
| `/strategy-scripts` | **現貨策略腳本** | 寫一支吃現貨 K 線的算式、試跑、存起來、回測。清單只列 K 線種類的策略腳本 |
| `/trading-strategies` | **交易策略** | 拼一份交易策略並回測它。新拼時選**行情種類**（K 線或合約行情，存過之後不能換），信號來源只挑得到同一種的策略腳本；合約交易策略另選**交易模式**，並在合約帳戶上重演（重演時只問槓桿與滑點）。清單標出每一份吃的行情。策略機器人只挑得到 K 線交易策略 |
| `/contract-strategy-scripts` | **合約策略腳本** | 同一塊工作區，寫的是吃**合約行情格**的算式：每一格除了成交價，還帶成交筆數、標記／指數／溢價指數三組開高低收、收盤時現行的資金費率、這一格內有沒有結算與持倉統計（沒有值一律為零）。標的從合約標的清單挑；清單只列合約行情種類。**回測在逐倉合約帳戶上重演**：多問槓桿（留白即一倍）、交易模式（多空反手／只做多／只做空）與滑點；成績單多出強平、資金費用（付出／收到）、做多做空各自的筆數與勝率、被交易規則擋下的開倉與維持保證金依據，明細多出槓桿、數量、保證金與資金費用 |
...
**但「沒有東西」不一定是不正常的。** 那四種狀態共同的前提是「沒有東西就是出事了」，
而台股接進來之後這個前提不再成立：收盤時沒有新資料是正常的，某些標的沒有即時更新也是正常的。
把正常的事塞進那四種，等於天天謊報一次故障，而真正的故障就淹沒在裡面。
所以它們各有各的說法，見下方「兩個市場」。
...
**每一個代號旁邊就寫著公司的名字**（`2330 台積電`），觀察清單、挑標的的選單與圖表標題
都一樣。名字由後端從行情來源記下來，畫面不翻譯也不編；加密貨幣沒有名字，就只寫代號，
後面不多一個空格。「怎麼唸一檔」只在交易標的轉成對外形狀時決定一次——三個畫面各接一份的話，
遲早會有一個在沒有名字的時候多一個空格。

選單上每一檔還帶著**它屬於哪個市場**與**有沒有即時更新**，並可以只看某一個市場。
兩件事都必須在**挑之前**就看得到——挑完才發現這一檔不會動，那個資訊就來得太晚了。

**市場鍵的意思是「我現在要看哪個市場」**，所以切過去就換到那個市場的標的上：
選著的那一檔本來就屬於它就不動，不屬於就換成該市場的第一檔，該市場一檔都沒有時
一檔都不選、圖上說明還沒有行情可以畫。留一檔別的市場的標的在選單裡，畫面就會說謊——
分頁寫著台股，圖上畫的是比特幣。
...
| 即時更新 | 一律有 | 只有後端給到名額的那幾檔 |
| 成交額、主動買入量、主動買入額 | 有 | **沒有這一項** |
| 「立刻更新」按鈕 | 沒有 | **有** |

**「沒有這一項」畫成一個破折號並淡化，不是 `0`。** 零是一個真的讀數——某五分鐘真的可以
一筆都沒成交。兩者長得一樣的話，看的人沒有任何辦法分辨，而且不會有任何地方報錯。
修改 K 線時，這個市場不報的那幾格留白而不是預填一個 0——填了一按儲存，
就把「這個市場沒有這一項」寫成了「它是零」。

**會收盤的市場，圖表上多一顆「立刻更新」。** 台股收盤之後，後端每五分鐘一輪已經沒有
東西可收了——而那正是最想看今天資料的時候。按一下就要後端把這一檔缺的補回來，
補完說補到幾根並重新畫一次。補到零根也會說出來（那是常見的答案：手上已經是最新的），
不說的話看的人分不出「按了沒事」與「按了沒反應」。

加密貨幣沒有這顆按鈕：它永不收盤，永遠只差一輪就跟上了，給它一顆只是讓人多按一次
去做本來就會發生的事。判準因此是**這個市場會不會收盤**，不是「現在有沒有開」——
後者在收盤的每個夜裡都成立，卻在加密貨幣身上永遠不成立。

**加進觀察清單時後端會自動補一次**，所以正常情況下不必按它。

**圖表上那一句話一次只說一句**，優先序是 **收盤中 ＞ 這一檔沒有即時更新 ＞ 即時已停止**：

- **收盤中**——這個市場現在不交易，圖不動是正常的，開盤會自己動起來。
- **這一檔沒有即時更新**——它沒拿到即時名額，資料五分鐘更新一次。**不會自己好。**
- **即時已停止**——連線斷了，系統正在自己接回來。**會自己好。**

後兩句必須分開：一句的意思是「等到明天也一樣」，一句是「等一下會自己好」。
給錯那一句，使用者會一直等一件不會發生的事。收盤排最前面，是因為收盤時即時本來就會停，
兩句都說出來會讓人以為出了兩件事。

**收盤與名額都由後端回答，畫面不自己推算**：畫面不知道哪幾天是休市日
（推算的結果會把國定假日說成故障），也不知道後端把即時名額給了誰
（猜的結果會替一張永遠不動的圖保證即時更新）。
...
這個操作台收在一道門後面：**沒登入就只看得到 `/login`**。

- 第一次來的人在同一張卡片上按「還沒有帳號？建立一個」開一個帳號，**建完直接就是登入狀態**。
- 登入後憑證記在**這台瀏覽器**上，重新整理、關掉分頁再打開都還是登入狀態。
- **好幾週不必重打密碼**：短命的登入憑證過期時，畫面自己拿長命的續用憑證去換一份，
  使用者看不到任何異狀（見下）。
- 側欄最底下寫著現在是誰在用，旁邊按一下就登出，而且**是真的登出**。
- **被門擋下來的人，登入之後會回到他本來要去的那一頁**，不是第一站（K 線圖表）。

需要後端 `go-trading` 設定 `AUTH_ACCESS_TOKEN_SIGNING_KEY`；沒設定時登入會回 503，
畫面會明講「這不是你填錯了什麼」，而不是說帳密不正確。

### 打開操作台時，能自己修好的就自己修

後端一次發**一對**憑證：登入憑證（15 分鐘）與續用憑證（30 天）。畫面兩份都記住，
然後每次打開時依序判斷：

| 記著的狀態 | 做什麼 |
| :--- | :--- |
| 什麼都沒有 | 沒登入。**不打後端** |
| 續用憑證也過期了 | 沒登入，丟掉記著的東西。**不打後端**——答案已經算得出來 |
| 兩份都還有效 | 直接問「我是誰」 |
| 登入憑證過期、續用憑證還在 | **先換一對新的**，再問「我是誰」。使用者看不到任何異狀 |
| 問了卻被拒絕（例如後端重啟換過鑰匙） | **試著換一次**；成功就重問，失敗就回登入畫面 |

**「試一次」就是一次。** 續用憑證只能用一次，一直重試會踩到後端的盜用偵測——
那會把「這台需要重登」升級成「這個人每一台裝置都被登出」。
所以那個界線寫在程式的結構裡，不是寫成一個可以調的重試次數。

### 登出是真的登出

按下登出會**先請後端撤掉這台裝置的登入階段**，再清掉本機。之後那份續用憑證換不到任何東西。

**後端連不上時，登出仍然成功**：本機照樣清乾淨、照樣回到登入畫面。
做不到的只是「立刻讓後端也忘記」，而使用者能做的只有稍後再登出一次——
那不值得把他攔在一個他已經決定要離開的畫面上。

仍然有一個尾巴：後端撤得掉續用憑證，但撤不掉已經發出去的登入憑證，
所以最長還有 15 分鐘（後端的 `AUTH_ACCESS_TOKEN_LIFETIME_MINUTES`）它仍然通得過。

### 送出之前擋下來的那幾條，不是規則的所在地

畫面自己會擋掉兩格空白，以及**建立帳號時**密碼少於 8 個字元、超過 72 個位元組。
那是為了讓人不必按了鍵、等一趟來回才知道密碼太短——**規則在後端，兩邊說法不同時後端說了算**。
**登入不套用長度規則**：密碼比今天的最短長度短，只代表它不對，而不是「你格式填錯了」；
真要套用的話，最短長度一改，昨天設得起來的密碼今天就登不進去。

電子郵件的格式**刻意不在這裡判**。抄一份後端的規則過來，只買到一趟省下的來回，
卻換來第二份遲早會漂移的規則。

### 這道門只在瀏覽器這一側

把關是全域中介層，而且**只在瀏覽器端跑**——伺服器算頁面時碰不到瀏覽器的儲存，
在那裡判斷必然得到「一律沒登入」，於是每次載入都會先閃一下登入畫面。

後端另有自己的門：**看行情的端點（查 K 線、看圖、跟盤、列交易標的）不問來者是誰**，
其餘——包含**改動行情**（新增／修改／刪除 K 線、立刻更新、歷史同步、追蹤名單）——
都要已放行的登入（見 `go-trading` 的 `.sdd/2026-09-25-market-data-writes-require-sign-in/`）。
操作台送出的每一發都帶著記著的身分、過期先自己救回一次，所以這些動作不必各自處理登入。
...
| 把外框收起來，讓圖佔滿版面 | [`.sdd/2026-09-05-collapsible-console-chrome/`](.sdd/2026-09-05-collapsible-console-chrome/) |
...
│   ├── molecules/          原子組成的功能單位（BackendStatusIndicator…）
...
├── middleware/             全域路由中介層（把關：沒登入就只看得到登入畫面）
